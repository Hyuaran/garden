-- Codex-187: employee work time and result-flag breakdown for the call metrics portal.
--
      ...
      greatest(0::numeric,
        ...
        - coalesce((
          select sum(extract(epoch from (
            least(daily.last_end, break_time.b_end)
            - greatest(daily.first_start, break_time.b_start)
          )))
          from breaks break_time
          where least(daily.last_end, break_time.b_end)
            > greatest(daily.first_start, break_time.b_start)
        ), 0::numeric)
      ) as work_seconds
