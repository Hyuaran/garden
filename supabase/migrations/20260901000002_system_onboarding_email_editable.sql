-- 提出後もメールアドレスだけは事務が入れられるようにする。
-- 提出ずみの人に、あとから項目（メールアドレス）を足したため、本人も事務も入れられない状態になっていた。
-- 保護そのものは変えず、除外リストに email を1つ加えるだけ。
begin;
...
comment on column public.system_onboarding.email is '本人のメールアドレス。提出後は事務が入れられる（他の本人入力項目は提出後に変更できない）。';
