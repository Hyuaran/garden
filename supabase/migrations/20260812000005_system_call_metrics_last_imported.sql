-- Add the latest imported_at timestamp to the existing four-argument RPC response.
...
comment on function public.system_call_metrics(date, date, text, text) is
  'Manager-only call metrics with exact-match cross-filters and latest imported_at.';

-- Verify after applying as an authenticated manager:
-- select public.system_call_metrics(current_date, current_date, null, null)->>'last_imported_at';
