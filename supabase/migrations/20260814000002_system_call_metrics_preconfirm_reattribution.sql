...
create or replace function public.system_call_metrics(
  ...
  with authorized as (
    ...
    order by coalesce(rollup.call_count, 0) desc, coalesce(rollup.employee_name, preconfirm.employee_name)
  )
  select case
...
comment on function public.system_call_metrics(date, date, text, text) is
  'Manager or service-role rollup metrics; 前確OK is attributed by external_sales_id to the deterministic acquisition owner/list.';
