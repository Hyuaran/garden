...
--    a-review #55 R2 修正 (2026-04-27 a-bud):
--    旧定義（statement_id PK / deposit_amount / withdrawal_amount / reconcile_*）は
--    A-06 で全面再設計されたため、本ファイルからは削除。
--    実定義は scripts/bud-a06-statements-migration.sql §2 を唯一の正本とする
--    （id uuid PK / amount signed / matched_transfer_id / raw_row jsonb / Phase A 採用設計）。
--
--    旧定義の問題: bud-schema.sql が先に適用されると、a06 の CREATE TABLE IF NOT EXISTS
--    が silent に no-op となり、新スキーマが喪失。TypeScript 側は a06 schema 前提で
--    クエリを発行しているため、bank_account_id / amount / matched_transfer_id 等の
--    列が見つからず全クエリ失敗（経理事故レベル）。
-- ============================================================
