...
| 2026-04-25 | 2026-04-25 (W1-W6) | Bud | Phase A-1 A-06 明細管理 + 自動照合 | 0.75 | 0.6 (W1-W6) | -0.15 | a-bud (A) | Claude | **W1-W6 完了**: SQL migration（bud_statements + bud_statement_import_batches + RLS）+ 楽天/みずほ/PayPay CSV パーサ（純関数、17 tests）+ 自動照合ロジック（exact/high の 4 段階フォールバック、13 tests）+ Server Action importBankStatements + statement-queries + /bud/statements 一覧 + ManualAssignModal + StatementImportModal。**自動照合成功時は振込側を 振込完了 に遷移**（A-03 transitionTransferStatus 連携）。272 tests 緑（+30）。**残: W7 月次集計 / W8 取込バッチ履歴 UI（合計 0.1d）→ ハンドオフ**。 |
