...
| Root | 権限管理画面 + Garden ヘルプモジュール 新規 spec 2 件起草（[a-root]） | 1.5 | 0.5 | -1.0 | a-root-002 (A) | 2026-04-26 | 2026-04-26 | spec-revision-followups-20260426 §3.1/§3.2 反映。新規ブランチ feature/root-permissions-and-help-specs（develop ベース）。subagent 並列起草（sonnet × 2、約 6 分）。①権限管理画面 1,004 行（root_role_permissions + root_user_permission_overrides の Kintone 風マトリックス UI、削除列ロック super_admin only、Tree のみ manager+ 編集可、has_permission_v2() で B-1 と統合）②Garden ヘルプモジュール 828 行（/help + /<module>/help ハイブリッド、権限連動可視範囲、Postgres FTS 検索、動画 Storage、お問い合わせ Chatwork 連携）。判断保留 15 件。実装は Phase D-E。push は GitHub 復旧後。 |
| Root | 権限管理画面 実装 (root_role_permissions + UI) | 2.25 | | | a-root (A) | | | spec: docs/specs/2026-04-26-root-permissions-management-ui.md。Kintone 風マトリックス + 個別 override + 申請承認パターン。Phase D-E 着手予定。 |
| Root | Garden ヘルプモジュール 実装 (/help + /<module>/help) | 2.5 | | | a-root (A) | | | spec: docs/specs/2026-04-26-root-help-module.md。記事 CRUD + Postgres FTS + 動画 Storage + お問い合わせフォーム。Phase D-E 着手予定。 |
| Root | 33 件判断保留消化 Cat 1+2 反映 + 開発者ページ spec 新規起草（[a-root]） | 1.5 | 0.7 | -0.8 | a-root-002 (A) | 2026-04-26 | 2026-04-26 | a-main 007 確定ログ (decisions-pending-batch-20260426) Cat 1 (8件) + Cat 2 (7件) 反映。新規ブランチ feature/root-pending-decisions-applied-20260426 (help-specs ベース)。subagent 並列 (sonnet × 3、約 8 分)。①B-01 spec +257/-16 行：「東海林さん」表現統一 + #5 outsource Leaf 既定 denied + #8 緊急ワンタイムキー新規 + 見積 2.25d→2.5d ②ヘルプ spec +454/-151 行：#10 編集禁止化（誰も直接編集しない、コメントのみ）+ #14 Chatwork+dev-inbox 2 経路通知 + 見積 2.5d→3.0d ③新規 dev-inbox spec 775 行：問い合わせ・依頼の一元管理画面 (/admin/dev-inbox)、Phase A-B Chatwork 運用、Phase C-D 本格実装、見積 2.25d。総差分 +1,486 行。push は GitHub 復旧後。 |
| Root | Phase B-01 実装更新: ワンタイムキー機能 (#8) 追加分 | 0.25 | | | a-root (A) | | | Cat 1 #8 緊急時超過権限。root_emergency_keys テーブル + has_permission_v2() 0 段階 + super_admin 専用 UI。Phase B 着手指示後、B-01 本体と同時実装推奨。 |
| Root | ヘルプ実装更新: コメント機能 + 開発者依頼ボタン (#10) 追加分 | 0.5 | | | a-root (A) | | | Cat 2 #10 編集禁止化に伴う追加分。root_help_article_comments + root_help_article_change_requests テーブル。Phase D-E ヘルプ本実装と同時。 |
| Root | 開発者ページ実装 (Cat 2 #14) | 2.25 | | | a-root (A) | | | spec: docs/specs/2026-04-26-root-developer-inbox.md。問い合わせ・依頼の一元管理画面 /admin/dev-inbox。Phase A-B は Chatwork のみ運用、Phase C-D 本格実装。 |
