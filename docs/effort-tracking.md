...
| Tree | Phase D 4 次 follow-up（Rill リネーム + 通知センター統合 + Softphone 後道さん FB 範囲）| 0.3 | 0.3 | 0 | a-tree (A) | 2026-04-26 | 2026-04-26 | (1) spec-tree-rill-rename.md 新規起草（v1 リネームのみ、v2 で全社 Chatwork クローン拡張ロードマップ、CLAUDE.md §1 更新提案）。(2) spec-tree-toast-notification.md v1.1 改訂（既存 KPIHeader 通知センター統合、独立 localStorage 廃止、当日全件保持 + 翌日リセット、addNotif/resetNotifsAtMidnight 追加）。(3) spec-tree-softphone-design.md v1.1 改訂（判 #3 モニタリング被モニタ通知デフォルト OFF + root_settings 切替 / 判 #4 後道さん FB 不要・架電画面例外明文化 / 判 #5 root_employees.softphone_position jsonb）。(4) claude-md-update-proposal-rill.md 起草（Rill 段階的進化モデルの §1 反映提案、a-main で実適用）。同ブランチ追加 commit。 |
| Tree | Phase D 5 次 follow-up（確定 10 件: #16 セットリリース + #17 50名 WS + #18 録音注記 + #19 商材 + #20 KPI 3 階層 + others）| 0.3 | 0.3 | 0 | a-tree (A) | 2026-04-26 | 2026-04-26 | a-main 006 33 件判断保留消化のうち Tree 担当 Cat 3 全 10 件を反映。**最重要 #16**: D-1+D-2 セットリリース戦略を plan v3 §4 に明示、ソフトフォン spec §0 にも追加、見積 6.5d → 8.5d (+2.0d)。**#17**: D-03 spec §0 に「30 秒 polling 既定 + 同時 50 名超で WebSocket 自動切替」追記。**#18**: D-01/D-03/Softphone spec に「イノベラ API 採用具合次第で Garden 内録音実装方針へ変更可能性あり」注記。**#19**: D-04 spec §0 に「D-1 関電 / D-2 光 / D-3 クレカ」明確化。**#20**: D-05 spec に KPI 目標 3 階層 DB 設計（tree_kpi_personal_targets 必須 / tree_kpi_team_targets 任意 NULL 許容 / tree_kpi_company_targets 必須）追加。#21-#25 は既反映確認のみ。同ブランチ追加 commit。 |
| Tree | Tree Breeze → Garden Rill v1 リネーム実装 | 0.07 | | | a-tree (A) | (pending) | | spec-tree-rill-rename.md §2.4 準拠、画面文言・ナビラベル・JSDoc を Garden Rill に変更（src/app/tree/breeze/ ファイルパスは v2 で移行のため v1 では維持）。Phase D-1 着手前に実施推奨。 |
| Rill | Rill v2 拡張（Phase 後期、Chatwork クローン全社チャット）| 別途算出 | | | a-rill (将来) | (pending) | | spec-tree-rill-rename.md §3 ロードマップ参照。Phase D-1 完了 + Root Phase B 完了後に着手判断。永続保存・全社展開・Chatwork API 通知の内製置換を含む。 |
