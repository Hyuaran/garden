...
| Root | Phase B-7 実装: 移行ツール（Kintone / FileMaker 取り込みヘルパー） | 4.0 | | | a-root (A) | | | spec: docs/specs/2026-04-25-root-phase-b-07-migration-tools.md。5 段階 (B-7.1 基盤 → B-7.2 Kintone → B-7.3 FileMaker → B-7.4 関電業務委託 Leaf 連携 → B-7.5 営業リスト Soil 大量取込)。最重量 Phase。**2026-04-26 Kintone 確定 #29/#32 反映で +1.5d (B-7.6 dual-write / B-7.7 切替手順 / B-7.8 解約手順) → 5.5d** |
| Root | Kintone 確定 6 件反映: spec 改訂 + 新規 B-08 起草 (#23/#24/#27/#28/#29/#31) | 0.5 | 0.5 | 0 | a-root-002 (A) | 2026-04-26 | 2026-04-26 | a-main 006 確定ログ (decisions-kintone-batch-20260426) を Root 担当 6 件分反映。新規: B-08 employees 拡張 spec (644 行、#23 打刻ID/KOTID 並列保持 + #24 team_history 構造 + trigger + #31 助成金/休業履歴 保管期間 10→7 年運用)。改訂: B-6 §2.5 (CW_API 未運用注記 #27) + B-7 §5.1/§11.5/§11.6 (Skip 対象フィールド #28 / merge 戦略 A #29 / Kintone 段階的解約フロー #32)。subagent 1 本 + 直接編集 2 ファイル。判断保留 12 件 (B-08 6 + B-7 既存 + 新規)。push は GitHub 復旧後。 |
| Root | Phase B-08 実装: employees 拡張 (打刻ID/KOTID 並列 + team_history + 助成金/休業履歴) | 2.5 | | | a-root (A) | | | spec: docs/specs/2026-04-26-root-phase-b-08-employees-extension-from-kintone.md。Kintone App 56 (105 fields + 4 SUBTABLE) 移行を見据えた root_employees 拡張。新規 4 テーブル + trigger + retention 自動削除 cron。 |
