...
| Bud | Phase E spec batch 起草（v1、5 件 + overview）— 給与処理拡張 + 勤怠取込拡張 + MFC 拡張 | 0.75 | 0.4 | -0.35 | a-bud-002 (A) | 2026-05-08 | 2026-05-08 | main- No. 136 GO 受領後 Phase D 100% 完成 (PR #148) → Phase E spec 起草フェーズ移行。develop 派生ブランチで起草（Batch 17 / 18 precedent 踏襲）。overview + 5 spec skeleton: E-01 月次運用 Cron + リマインダ拡張（0.75d、Vercel Cron 3 系統 + リトライ + 設定 UI）、E-02 退職者・中途入社特例（1.0d、calculateProratedDays / decideInsuranceProration / calculateImmediateSettlement + bud_special_case_log + D-02/D-05/D-06 統合）、E-03 産休育休フロー（0.75d、isInsuranceExempted / detectChildcareReturn / judgeChildcareGetsuhen / applyChildcarePreservedRemuneration + bud_employee_preserved_grade_applications）、E-04 役員報酬区分（0.5d、judgeOfficerCompensation / classifyOfficerToAccountingCategory + bud_officer_compensation_settings + disallowed_amount_log + D-07/D-11 統合）、E-05 KoT 完全同期 + Excel 取込（1.0d、`/api/cron/bud-kot-monthly-sync` + parseAttendanceExcel + bud_attendance_correction_requests + manager → payroll_calculator 申請ワークフロー）。実装見積合計 4.0d（5 件）、本起草工数 0.4d（5 spec + overview = ~1900 行）。spec 100% 共通制約遵守（設計判断・仕様変更なし、新規 npm install なし、動作変更なし、既存 helpers 再利用、precedent 踏襲）、判断保留事項を各 spec 6-7 件列挙し東海林さん判断仰ぎ。レビュー指定: a-bloom。 |
