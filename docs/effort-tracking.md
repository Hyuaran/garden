...
| 2026-04-22 | 2026-04-22 | Tree | Phase A Supabase認証・権限連携 | 0.5 | 0.5 | 0 | a-main (A) | Claude | 遡及記録（実績は commit 履歴 13:22〜13:53 の範囲から推定）。社員番号+擬似メール方式、garden_role 7段階、TreeAuthGate/ログアウトボタン/氏名ハードコード除去。|
...
| 2026-04-24 | 2026-04-24 | Tree | Phase B-β 仕様書起草（誕生日同期 / 擬似メール変換 / bare screen） | 0.25 | 0.2 | -0.05 | a-tree | Claude | a-main B案採択。`docs/specs/2026-04-24-tree-phase-b-beta-birthday-password.md` 新規作成（6章+付録A、🔴2件を未決事項として明記）。§2.5 の部分成功復帰は次フェーズ spec で確定方針。派生 `feature/tree-review-20260424-auto` は保留継続。予定 0.25d (2h) → 実績 0.2d (約 95分)、章立てが前提洗い出し時点で固まっていたため順調。|
| 2026-04-24 | 2026-04-24 | Tree | Phase B-β ブラウザE2E手順書（三好理央 1324） | 0.0625 | 0.08 | +0.02 | a-tree | Claude | `docs/phase-b-beta-e2e-checklist.md` 新規作成（STEP1〜11 + 11項目チェックリスト + 後片付け SQL）。非技術者向け、Supabase Dashboard 操作と `npm run dev` 起動を含む全手順を明記。予定 0.0625d (30分) → 実績 0.08d (約 40分)、チェックリスト化と失敗時対処も盛り込んで +0.02d。|
| 2026-04-24 | 2026-04-24 | Tree | Phase B-β 仕様書 v1.1 改訂 + E2E B 経路追加（a-main ③確定方針反映） | 0.25 | 0.2 | -0.05 | a-tree | Claude | a-main 回答：①マイページ誕生日変更時も同期する、②パス忘れ再発行は Root 側 UI で対応・`password_reset_min_role` で設定変更可能・初期値 manager・admin 変更可・audit log 記録。仕様書 §2.1/§2.2.2/§2.4/§2.5/§2.6 全面更新、§5 を確定事項/未決事項に分離、§6 に v1.1 追記、付録A に次フェーズ実装予定 5件追加。E2E に STEP 12-15（次フェーズ有効化）追加。MEMORY.md `project_configurable_permission_policies` パターン準拠。予定 0.25d → 実績 0.2d (-0.05d)、self-review まで含めて順調。|
