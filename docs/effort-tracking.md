...
| Soil | Phase B-01 Phase 2 実装 7 タスク完走（FileMaker CSV 200 万件取込 / Batch 20 spec → 実装） | 1.8 | 0.2 | -1.6 | a-soil-002 (A) | 2026-05-09 | 2026-05-09 | spec 1.8d → 実績 ~1.2h で完走（-90%、Phase 1 既存実装の Adapter Pattern 再利用が極めて効率的）。**新規 4 lib + 1 script + 1 sample-gen + 1 migration + 1 SQL + 1 runbook + 97 tests 追加**: ①soil-csv-parser.ts（自前 readline、RFC 4180 minimal、BOM/CRLF/multi-line cells 対応、36 tests）②soil-import-csv-source.ts（Adapter: CsvRow → KintoneApp55Record + Phase 2 transform 上書き、31 tests）③soil-merge-detector.ts（R1/R2/R3 重複検出純粋関数、19 tests）④soil-csv-importer.ts（runSoilImportFromCsv orchestrator、loadNormalizedToSoilLists 再利用、7 tests）⑤scripts/soil-import-csv-phase2.ts（Node.js 実行スクリプト、ストリーミング読込、200 万件対応）⑥scripts/soil-generate-sample-csv.ts（α テスト用 200 件合成 CSV 生成）⑦migration 20260509000001_soil_phase2_index_helpers.sql（admin 専用 INDEX OFF helper 関数 2 件 + 権限）⑧scripts/soil-phase2-recreate-indexes.sql（CONCURRENTLY 12 INDEX 再構築）⑨docs/runbooks/filemaker-export-runbook.md（運用手順書、エクスポート / 取込 / トラブルシューティング / ロールバック）⑩admin UI 拡張（page.tsx に source filter + Phase 2 ジョブ作成フォーム + script コマンド表示）⑪createPhase2ImportJob server action（4 tests）。Phase 2 関連 db tests 全 97/97 PASS、soil db 全 181/181 PASS（Phase 1 + Phase 2）。ブランチ `feature/soil-batch20-impl`（feature/soil-batch16-impl 派生）。migrations apply は 5/12 推奨（5/13 統合テスト前日）、α テストは sample CSV 200 件 → 200 万件本番想定 |
