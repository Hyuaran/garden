...
| Bud | Phase D-04: 給与明細配信（PDF + Tree マイページ + メール PW PDF） | 1.25 | — | — | a-bud + a-tree / a-auto 005 spec | 2026-04-25 | — | a-auto 005 spec 起草。A-07 採択反映: 主経路 = Tree マイページ（A 案）、補助経路 = メール PW PDF（方式 2）。bud_salary_notifications + 自動再送 1h/6h/24h。pdf-lib + Resend 想定（要 npm 承認）。現金手渡し受給者の受領確認ボタン。SHA256 改ざん検知。労基法 24・電子帳簿保存法準拠。 |
...
| Bud | Phase D-09: 口座一覧 (Kintone App 92 → 口座分離設計) | 0.75 | — | — | a-bud (A) | 2026-04-26 | — | Kintone 解析 #16 反映。employee_bank_accounts (全従業員、給与口座) + payment_recipients (外部支払先 + 特殊扱い 10 名、月変動、employee_id NULL 可)。EXCLUDE 制約で 1 従業員 1 アクティブ口座。bud.has_payroll_role() ヘルパー関数を D-09 §4 で先行定義し他 D-* spec から再利用。dual-write 1〜2 ヶ月 (#32 解約戦略準拠)。判保留 5 件 (特殊扱い 10 名リスト確定 等)。新規 spec 起草、local commit。 |
| Bud | Phase D-10: 給与計算統合 (Excel 排除 + Tree 源泉 + Bloom 集計連携) | 2.5 | — | — | a-bud (A) | 2026-04-26 | — | Kintone 解析 #17 + #20 反映。「【月1】給与計算用」+ インセン計算シートを Bud に完全移植、Excel 廃止。インセン 5 種 (AP / 件数 / 社長賞 / チーム勝利金 / P 達成金) 純関数化、Excel 結果と完全一致テスト 90+ 想定。Tree 源泉 (kpi_summary_for_bud ビュー、a-tree 協議要) + Bloom 集計連携。bud_payroll_records (4 段階ステータス + 監査履歴) + V6 自己承認禁止と同等の「承認者≠計算者」RLS。新規 spec 起草、local commit。 |
| Bud | Phase D-11: MFC 互換 CSV 出力 (72 列 / cp932 / 9 カテゴリ) | 1.5 | — | — | a-bud (A) | 2026-04-26 | — | Kintone 解析 #19 反映。memory project_mfc_payroll_csv_format.md を正本として 72 列マッパー実装。9 カテゴリ (識別 / 所属 / 所定 / 勤怠 / 月給 / 時給 / 日給 / 控除 / 備考)。3 形態 (月給 / 時給 / 日給) 混在対応。bud_mfc_csv_exports + bud_mfc_csv_export_items (jsonb スナップショット)。Storage パス uuid 化、admin only DL (D-04 と同パターン)。承認者≠生成者 RLS。サンプル CSV と byte 一致確認テスト。新規 spec 起草、local commit。 |
| Bud | Kintone 解析 #18 + #25 反映: 既存 D-01〜03/05〜08 spec へ統一注記追加 (local commit) | 0.15 | 0.15 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | #18 payroll 系 4 ロール (calculator/approver/disburser/auditor) を D-01/02/03/05/06/07/08 末尾に統一注記として追記。実装は bud.has_payroll_role() ヘルパー (D-09 §4 定義) を再利用。#25 Kintone App 21「東海林頼んだ Excel」フィールドは Garden 移行時に廃止、bud_payroll_records (D-10) が直接 master、月次報告資料 Excel への参照不要を明記。D-04 は A-07 改訂で別管理のため対象外。 |
| Bud | follow-up #1: D-01 給与期間 大幅修正 (1 日〜月末 / 翌月末営業日 / root_settings 化) | 0.1 | 0.1 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | 東海林さん再判断で元 a-auto 推奨「当月 21 日〜翌月 20 日 (KoT 既存運用)」を全面改訂。勤怠締め=1 日〜月末、給与支払=翌月末最終営業日、明細配信=20-25 日。root_settings 化 (kintai_period_start_day / end_day / payment_date_rule / statement_publish_target) で admin が再起動なしで変更可。bud_payroll_periods に statement_publish_target_date 列追加。判保留 +2 件追加 (root_settings 履歴管理 / 明細自動配信化)。Phase E で自動 Cron 化対応設計。 |
| Bud | follow-up #2: D-06 年末調整精算月 = 1 月精算へ変更 (12 月精算から) | 0.05 | 0.05 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | 東海林さん再判断で 12 月精算 → 翌年 1 月精算に変更。理由: 年末手取り安定 + 賞与分離 + 退職者対応シンプル化。§2 全体フロー改訂、§5 「12 月給与での精算」→「翌年 1 月給与での精算」改題、§10 判 1 を 1 月精算へ書換、§11.4 還付額警告閾値の運用判断追加 (gross_pay 30% → 20% 引き下げ検討)、§11.5 退職者ルール簡素化 (12 月末退職者は最終給与で即時精算、1 月精算対象外)。判保留 +1 件追加 (1 月精算時の月次源泉計算順序)。 |
| Bud | follow-up #3: D-07 振込ファイル = ハイブリッド方式へ変更 (30 件閾値廃止) | 0.15 | 0.15 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | 東海林さん再判断で「30 件閾値で個別振込/FB 切替」をハイブリッド方式に統一。銀行への振込=FB データ 1 ファイル一括 (件数によらず手数料 1 取引分)、Garden 側で勘定項目別レポート CSV 生成 (給与/外注費/通勤手当/賞与/役員報酬 5 区分)、後道さんがマネーフォワードクラウド会計取込で勘定項目別仕訳。§2 全面改訂、§3.3 bud_payroll_accounting_reports テーブル新設 (category_breakdown jsonb)、§3.4 CSV フォーマット定義 (UTF-8 BOM 付き、MFC 取込形式)、§4.1 連携フローをハイブリッド出力に書換、§10 判 1 を 30 件閾値廃止に更新。レポート粒度 5 区分は後道さん最終確認 + Phase E で粒度調整可 (判保留追加)。 |
| Bud | follow-up 2 次: D-07 §3.3 勘定項目別レポートを 5 区分 → 8 区分階層構造へ修正 + MFC 取込責任者を東海林に変更 | 0.15 | 0.15 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | 1 次 follow-up で仮置きした 5 区分フラットを実運用 Excel「グループ6社 年間コスト ワンビュー」シート 10 ベースの 8 大区分階層へ全面修正。category_breakdown jsonb → category_hierarchy jsonb（items[]/subtotal/is_future_use 構造）。8 区分: 役員報酬 / 給与 / 賞与(将来枠) / 交通費 / 会社負担社保等 / 外注費 / 販売促進費 / 固定費等。CSV を階層形式（明細行 + 小計行 + 総合計「役員給与系を除く」）に書換。MFC 取込責任者を「後道さん」→「東海林さん（admin）」に変更（後道さんは Garden レポート + MFC 取込結果確認のみ）。後道さんへの仕様確認は UI 完成後フィードバック方式（feedback_ui_first_then_postcheck_with_godo.md 準拠）。テーブル列追加: shared_with_godo_at / shared_with_godo_by。判保留 +0、判 1 を 2 次採択に更新。 |
| Bud | follow-up 3 次: D-10/D-11 給与確定フローを 4 段階 → 6 段階に拡張 + D-12 スケジュール/リマインダ新規起草 | 0.3 | 0.3 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | 東海林さん指摘の実運用フロー（上田→宮永小泉→上田→東海林→社労士→東海林）を反映、4 段階 → 6 段階へ拡張。新 status: confirmed_by_auditor（東海林目視）/ confirmed_by_sharoshi（社労士 OK 後マーク）。D-10/D-11 status enum 拡張、テーブル列追加（sharoshi_request_sent_at / sharoshi_partner_id / confirmed_by_*）、RLS 6 段階遷移 + V6 自起票禁止 + 巻き戻し policy。社労士は root_partners 外部取引先（Garden ログイン不要）、東海林さんが Garden 上で代理マーク。Server Action 6 種に拡張（confirmByAuditor/requestSharoshiConfirmation/markSharoshiConfirmed/finalizePayroll 追加）。新規 D-12 spec 起草: bud_payroll_schedule + payroll_schedule_settings + payroll_reminder_log、日次 Cron 09:00 JST、Chatwork DM + Garden Toast、重要度（info/warning/critical）+ エスカレーション 3 段階（通常/東海林DM/全社員）、admin 設定画面（営業日 offset 6 stage / 担当者デフォルト / 閾値）。判保留 7 件（祝日対応 / 全社員通知トーン / 営業時間カウント 等）。 |
| Bud | follow-up 4 次: 33 件判断保留 Cat 4 #26/#27/#28 反映（後道不在 + 上田目視ダブルチェック + 振込同時出力 + 賞与 admin 限定） | 0.4 | 0.4 | 0 | a-bud (A) | 2026-04-26 | 2026-04-26 | 確定根拠: decisions-pending-batch-20260426.md Cat 4。**#26（修正）**: 給与確認フロー = 後道さん不在、上田君目視ダブルチェック + 東海林さん振込。D-10/D-11 を 6 → 7 段階拡張、新 status='visual_double_checked' 挿入（confirmed_by_auditor と confirmed_by_sharoshi の間）。新ロール `payroll_visual_checker`（上田）追加 = 5 ロール体制。新列: visual_double_check_requested_at / visual_double_checked_at / visual_double_checked_by / visual_check_note。新 RLS policy: pr_visual_check（上田 SELECT + 当該遷移のみ、編集権なし）+ pr_request_visual_check（東海林依頼）+ 巻き戻し対象に visual_double_checked 追加。Server Action +2 種（requestVisualDoubleCheck / markVisualDoubleChecked）= 8 種体制。**上田用 UI 要件**を D-04 §2.7 に正本化（シンプル / 大きく見やすい / timeout なし / 閲覧 + OK ボタンのみ / 編集権なし）+ /bud/payroll/visual-check 画面 URL + UI モック。D-12 schedule に visual_double_check stage 追加（7 stage、+1 営業日 offset）+ default_visual_checker_id 列 + 上田向けリマインダ文面。**#27（A）**: MFC CSV 出力 = 振込ファイル生成と同時。D-07 §4.4 で 3 経路同時生成（経路 A FB / 経路 B 会計レポート / 経路 C MFC CSV）を exportPayrollBatchHybrid() で統一、D-11 §1.4 同時生成タイミング明記。**#28（A）**: 賞与処理 admin のみ承認・配信。D-03 §7 RLS を admin only INSERT/UPDATE 化、§7.1 運用境界表追記、D-10 §11 賞与処理 admin 限定セクション追加。後道さん不在を D-10 §6.2 / D-04 §2.7 / D-07 §4.1 で明示。見積影響: D-10 +0.4d / D-04 +0.3d / D-07 +0.2d / D-12 +0.05d（合計 +0.95d 工数）、本作業実績 0.4d。push 待ち（GitHub suspended、a-main がまとめて push 予定）。 |
