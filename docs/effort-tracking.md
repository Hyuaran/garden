...
| Leaf | A-1c-spec/plan v3 + v3.1: spec v3 (1637行) + plan v3.1 (4457行、判2/判3 jsdom + vitest 4.x) | — | 1.5 | — | a-leaf (A) | 2026-04-22 | 2026-04-25 | v1→v2→v3→v3.1 改訂。a-bloom 技術 §2🟢/§4🟢/§7🟡/§8🟢、東海林さん業務 5 項目 OK。**PR #58 develop merge 済** |
| Leaf | A-1c Phase 0: Task 0.2 npm 5 個追加 + Task 0.3 vitest 確認 | 0.3 | 0.1 | -0.2 | a-leaf (A) + 東海林さん | 2026-04-25 | 2026-04-25 | 0.2 は東海林さん手動実行 (commit 27e31c0)。0.3 は既存 develop で no-op |
| Leaf | A-1c Task D.1: migration SQL v3 (scripts + supabase/migrations + runbook 3 ファイル) | 0.5 | 0.2 | -0.3 | a-leaf (A) | 2026-04-25 | 2026-04-25 | spec §8 v3 全量。**PR #65 発行済**、Dashboard 実行は東海林さん作業 (runbook §1-§5) |
| Leaf | A-1c Task D.2: src/lib/supabase/client.ts (横断共通 browser client) | 0.1 | 0.05 | -0.05 | a-leaf (A) | 2026-04-25 | 2026-04-25 | spec-cross-rls-audit §2 パターン A 準拠。**PR #66 発行済** |
| Leaf | A-1c Task D.3: types.ts 新規 + v3 拡張型 (GardenRole/LeafBusiness/UserBusiness/AttachmentHistory 等) | 0.15 | 0.1 | -0.05 | a-leaf (A) | 2026-04-25 | 2026-04-25 | feature/leaf-kanden-supabase-connect から取得 + v3 拡張で 282 行。**PR #67 発行済** |
| Leaf | A-1c Task D.4: kanden-storage-paths.ts (3 定数 + 4 関数) + TDD 12 件 | 0.2 | 0.1 | -0.1 | a-leaf (A) | 2026-04-25 | 2026-04-25 | pure function、JSDoc 完備、@example 付き。**PR #68 発行済** |
| Leaf | A-1c Task D.5: leaf-supabase-mock.ts (Leaf 専用 mock) + 動作テスト 11 件 | 0.2 | 0.15 | -0.05 | a-leaf (A) | 2026-04-25 | 2026-04-25 | 既存 supabase-mock.ts は破壊的変更なし、Leaf 固有 mock を分離 (plan v3 §4.4)。**PR #69 発行済** |
| Leaf | A-1c Task D.6+D.7: image-compression.ts + Web Worker (HEIC + Canvas) + テスト 19 件 | 0.4 | 0.25 | -0.15 | a-leaf (A) | 2026-04-25 | 2026-04-25 | main thread 版 + Worker ラッパ、heic2any 動的 import。Worker 不可環境フォールバックあり。**PR #70 発行済**（D.6/D.7 統合） |
| Leaf | A-1c Task D.8-D.11+D.13: attachments.ts 13 関数 + テスト 16 件 | 0.7 | 0.4 | -0.3 | a-leaf (A) | 2026-04-25 | 2026-04-25 | softDelete/undoSoftDelete/restoreAttachment/hardDeleteAttachment/upload/signedURL/fetchByCase/verifyDownloadPassword 等 13 関数を attachments.ts 単一ファイルで統合 (実装の自然な単位、前例 Task 0.2+0.3 / D.6+D.7)。**PR #72 発行済** |
| Leaf | A-1c Task D.12: role-context.tsx (Garden Role の React Context) + テスト 4 件 | 0.15 | 0.1 | -0.05 | a-leaf (A) | 2026-04-25 | 2026-04-25 | RoleProvider + useGardenRole フック。SSR 不可ため 'use client'。**PR #73 発行済** |
