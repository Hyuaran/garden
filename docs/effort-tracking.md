...
| Forest | T-F9 採用差分実装 (D2 sticky / D4 glow / D8 initial scroll / D10 zantei) | 0.65 | 0.15 | -0.5 | a-forest (A) | 2026-04-25 | 2026-04-25 | 判5 「a-forest 推奨どおり 4 件採用」確定で着手。判6 (D6 不採用) で他 D 項目スキップ。MicroGrid.test.tsx 新規 10 ケース TDD、stickyBg 定数追加、globals.css に @keyframes shinkou-glow 追加。RED 8 → GREEN 641/641 + build OK。PR #62 発行。事前 audit (PR #56) で照合済 + spec 通り実装で大幅短縮。 |
| Forest | T-F5 閲覧 (TaxFilesList + 4 components + migration) | 1.85 | 0.4 | -1.45 | a-forest (A) | 2026-04-25 | 2026-04-25 | Storage 戦略ハイブリッド採択（候補 1 手動アップ + 候補 2 Drive API は Phase B）受けて閲覧側のみ実装。migration 2 件 + types + queries + 4 components + state 統合 + dashboard マウント。spec の Tailwind 例は F3 採択（インライン継続）に従い C.* / FOREST_THEME を使ったインラインに置換。TDD 37 ケース新規（queries 9 + Icon 10 + Badge 3 + Group 9 + List 6）。RED 順次 → GREEN 全 668/668 passed。PR #64 発行。アップロード UI は判5 B 案で Phase B Storage 統合へ移行済（対象外）。 |
