...
| 2026-04-25 | (pending) | Bud | Phase A-1 A-03 振込 6 段階遷移（差分実装） | 0.3-0.5 | — | — | a-bud (A) | Claude | **予定**: Phase 1b.1 Foundation の transfer-status.ts / RLS v2 との差分で `bud_transfer_status_history` テーブル + `bud_can_transition` PL/pgSQL 関数 + `bud_transition_transfer_status` RPC + Server Action + Chatwork 即時通知を追加。監査は root_audit_log と二重記録（東海林判断 A-03 判1）。super_admin 自起票 reason は「自起票」固定（A-03 判3）。TDD（PR #33 マージ済 Vitest 利用可）。 |
