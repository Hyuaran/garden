...
| 2026-04-25 | 2026-04-25 (W1-W4) | Bud | Phase A-1 A-03 振込 6 段階遷移（差分実装） | 0.3-0.5 | 0.3 (W1-W4) | -0.0〜-0.2 | a-bud (A) | Claude | **W1-W4 完了 commit 09c30bc**: SQL migration（history テーブル + bud_can_transition + bud_transition_transfer_status RPC + RLS）+ canTransitionWithRole + transitionTransferStatus TS wrapper + 純関数バリデータ（transition-validator）+ Vitest 37 件追加（全 181 件緑）。super_admin 自起票 reason='自起票' 自動挿入（A-03 判3）。**残: W5 Chatwork 通知（0.1d、Phase A 内で別作業）/ root_audit_log 二重記録（A-03 判1、Phase B 提案）/ 東海林さん SQL 適用 → UI 側動作確認**。 |
| 2026-04-25 | (pending) | Bud | Phase A-1 A-04 振込 新規作成フォーム | 0.5 | — | — | a-bud (A) | Claude | **予定**: new-regular / new-cashback を実体化、TransferFormRegular / DuplicateWarning / BankPicker / KanaPreview / 新規取引先モーダル（A-04 判1）。当日振込禁止（A-04 判2）。旧 Phase 1b.2 Task 6 相当の再構成版。 |
| 2026-04-25 | (pending) | Bud | Phase A-1 A-05 振込 承認フロー UI | 0.5 | — | — | a-bud (A) | Claude | **予定**: 詳細画面 + StatusActionButtons + 一括操作 + CSV 出力骨格。本人承認は警告のみ（A-05 判1）。旧 Phase 1b.2 Task 9-10 相当の再構成版。 |
| 2026-04-26 | (pending) | Bud | Phase A-1 A-06 明細管理 + 自動照合 | 0.75 | — | — | a-bud (A) | Claude | **予定**: bud_statements 投入 + 楽天・みずほ CSV パーサ（A-06 判1）+ 自動照合 UI。 |
