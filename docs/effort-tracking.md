...
| Soil | Phase B-03 大幅修正（東海林さん指摘で前提見直し）| 0.25 | 0.25 | 0 | a-auto 011 / a-main 006 | 2026-04-26 | 2026-04-26 | follow-up §1.2 反映、6 件改訂で全面書換。Soil = master / Leaf = ミラー の関係明記（概念図 + データフロー図追加）。R1 = supply_point_22（22 桁 不変 ID）最優先、R2 以降 phone+name に降格。「飛び込み案件」用語廃止 → is_outside_list フラグ + source_channel 列追加。月次補完バッチ廃止 → 毎日深夜 03:00 リスト外取込（B-02 と統一）。orphan 警告閾値: テストレコード以外で 1 件即警告（high + super_admin DM）。case_type enum 4 値（latest/replaced/makinaoshi/outside）+ handle_pd_number_change() DB 関数で同一 supply_point_22 の pd_number 変更時に Trx 内整合更新。 |
| Soil | Phase B-06 重要修正（Leaf 商材別 + Tree → Leaf ミラー + toss 切替 + 槙さん例外）| 0.25 | 0.25 | 0 | a-auto 011 / a-main 006 | 2026-04-26 | 2026-04-26 | follow-up §1.5 反映、§17 新設で 4 件追加。商材別アクセスマトリックス（Tree: 全ロール / Leaf 関電: closer/toss 除外 / Leaf その他: outsource も除外）。Tree → Leaf ミラー出しデータフロー（leaf_kanden_with_tree_context VIEW、LATERAL JOIN、コピーなしリアルタイム参照）。toss_view_scope を root_settings で動的切替（self_only/team_all/all）、初期 self_only でリリース後フィードバックで拡大可。槙さん例外 = outsource + module_owner_flags->>'leaf_kanden'='true' で関電全件アクセス（jsonb 構造で他 module 拡張可）。a-root 連携要請 5 件（root_settings カテゴリ + module_owner_flags 列 + get_toss_view_scope() 関数 + 設定 UI 2 種）。 |
| Soil | Phase B-02 軽微修正（投入時刻 + アーカイブ将来拡張）| 0.1 | 0.1 | 0 | a-auto 012 / a-main 006 | 2026-04-26 | 2026-04-26 | follow-up §1.1 反映、判 1 / 判 3 / §8.3 改訂。投入時刻 = 毎日深夜 03-04 時開始 / 朝 06-07 時完了（cron `0 3-6 * * *`）、335 万件規模で複数夜分割視野。アーカイブ機能は当面なし、将来拡張可能な hooks（is_archive_eligible_partition() IMMUTABLE）残存。 |
| Soil | Phase B-04 軽微修正（MV REFRESH 1h 8-20 時）| 0.1 | 0.1 | 0 | a-auto 012 / a-main 006 | 2026-04-26 | 2026-04-26 | follow-up §1.3 反映、§7 改訂で MV REFRESH スケジュール変更。1 時間ごと、朝 8 時 START / 夜 20 時 END、13 回/日、夜間停止（cron `0 8-20 * * *`）。Soil 投入バッチ（B-02 03:00）と棲み分け。対象 MV を Leaf 関電 KPI 集計に明示（leaf_kanden_kpi_summary / revenue_monthly）。手動 force=1 例外実装。 |
| Soil | Phase B-05 軽微修正（R2 → GDrive 永続 + 訓練 YAGNI + 槙さん例外 + ポップアップ + Pro 維持）| 0.25 | 0.25 | 0 | a-auto 012 / a-main 006 | 2026-04-26 | 2026-04-26 | follow-up §1.4 反映、5 件改訂。§5.4-5.5 R2 12 ヶ月超を Google Drive 自動移管 cron 新設（毎月 1 日 04:00、Workspace 内永続保管、追加コスト 0、移管ログテーブル）。§8 リストア訓練 YAGNI 降格（手順書整備優先、年次必須化せず四半期セルフレビューのみ必須）。§6.4 槙さん権限例外（B-06 §17.4 連携、Leaf 関電のみ復元可、can_restore_leaf_kanden() 関数 + Chatwork DM 必須）。§11.3 復元時ポップアップ強制入力禁止（DB + UI 両層、is_module_in_restore() 関数 + RestoreInProgressGuard）。§10.3 Supabase Pro 維持（Team 昇格は Phase B-1 完了 + 業務影響評価後）。 |
