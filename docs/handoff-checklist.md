...
> 引越し起草前に上から順に必ず実施
> **成果物テンプレ準拠**: handoff-template.md / handoff-template-with-sample.md（正典サンプル）に沿って起草
...
| 区分 | ファイル | 用途 |
|---|---|---|
| **作業手順（本ファイル）** | handoff-checklist.md | 引越し前に前期セッションが順番に実施するチェックリスト |
| **成果物テンプレ** | handoff-template.md | 引越し成果物 handoff-a-main-N-to-(N+1)-*.md の正典フォーマット |
| **正典サンプル** | handoff-template-with-sample.md | 017→018 後追い版を実例として併走 |

引越し成果物を起草する際は、**handoff-template.md の §0 / §A / §B / §1-§13 構造に必ず準拠**。新セッション側で §0 ロックが作動するため、§A / §B / §12 が欠落していると起動 ritual が機能しない。
...
| **成果物テンプレ準拠** | 「handoff-template.md / with-sample.md 構造（§0 / §A / §B / §1-§13）で起草しました、構造抜けないか確認お願いします」 |
| **§A 前期重要決定 7-10 件** | 「重要決定 N 件を §A に書きました、各々経緯 / 結果 / 代替案 + 却下理由 / 適用範囲 / 体内化確認 5 セクション完備、抜けないか確認お願いします」 |
| **§B 前期違反 + マッピング表** | 「違反 N 件をマッピング表 + 詳細で §B に書きました、再発防止策具体動作（handoff §X / memory §Y / sentinel §Z 参照）明記、抜けないか確認お願いします」 |
| **§12 a-memory 連携明記** | 「a-memory 連携プロトコル（役割線引き / dispatch flow / キャッシュ機構 / bypass / エスカレーション）を §12 に書きました、抜けないか確認お願いします」 |
| 起動指示 | 「a-main-(N+1) を新 PowerShell で起動してください、起動時 §0 ロック作動 = 8 項目チェックリスト完了 + 東海林さん最終 GO 受領まで dispatch 禁止」 |
