...
## 13. Kintone 確定反映: 決定 #5 ◇マーク = 自動計算列を GENERATED で 1 列統合

> **改訂背景**: a-main 006 確定ログ #5 を反映。Kintone の ◇マーク（regex 等で他列から自動計算される列）を、Garden Fruit では **GENERATED ALWAYS AS ... STORED** または **trigger** で 1 列統合。重複保持不要。

### 13.1 方針

| Kintone | Garden Fruit |
|---|---|
| ◇法人番号フォーマット（13 桁）| GENERATED 列で normalize（ハイフン除去等）|
| ◇インボイス登録番号フォーマット（T+13 桁）| GENERATED 列で T プレフィックス検証 + normalize |
| ◇法人名カナ（半角化 / 法人格除去）| trigger で normalize、別列保持しない |
| ◇代表者氏名カナ | 同上 |
| ◇電話番号フォーマット | trigger で normalize（ハイフン統一）|

### 13.2 fruit_companies_legal の自動計算列

```sql
ALTER TABLE fruit_companies_legal
  -- 法人番号 normalized（数字のみ 13 桁）
  ADD COLUMN corporate_number_normalized text
    GENERATED ALWAYS AS (
      regexp_replace(coalesce(corporate_number, ''), '[^0-9]', '', 'g')
    ) STORED,

  -- インボイス登録番号 normalized（T + 13 桁）
  ADD COLUMN invoice_number_normalized text
    GENERATED ALWAYS AS (
      CASE
        WHEN invoice_number IS NULL THEN NULL
        WHEN invoice_number ~ '^T[0-9]{13}$' THEN invoice_number
        ELSE 'T' || regexp_replace(invoice_number, '[^0-9]', '', 'g')
      END
    ) STORED,

  -- 法人名 search 用（漢字 / カナ 統合 tsvector）
...
CREATE INDEX idx_fruit_companies_corp_num_normalized
  ON fruit_companies_legal (corporate_number_normalized)
  WHERE corporate_number_normalized != '';

CREATE INDEX idx_fruit_companies_invoice_normalized
  ON fruit_companies_legal (invoice_number_normalized);

CREATE INDEX idx_fruit_companies_name_search
  ON fruit_companies_legal USING gin (name_search);
```

### 13.3 trigger 実装（GENERATED 不可ケース）

電話番号 normalize / カナ統一など **複雑な変換** は trigger:

```sql
CREATE OR REPLACE FUNCTION trg_fruit_companies_legal_normalize()
RETURNS trigger
  LANGUAGE plpgsql
  SECURITY INVOKER
...
AS $$
BEGIN
  -- 電話番号 normalize（ハイフン除去 + 国際表記）
  IF NEW.phone IS NOT NULL THEN
    NEW.phone := regexp_replace(NEW.phone, '[\s\-\(\)]', '', 'g');
    NEW.phone := regexp_replace(NEW.phone, '^\+81', '0', 'g');
  END IF;

  -- カナ全角化（必要に応じて zenkana 関数を別途定義）
  IF NEW.name_kana IS NOT NULL THEN
    -- pg-trgm + custom hankaku→zenkaku 変換（実装時関数定義）
    NEW.name_kana := normalize_kana_zenkaku(NEW.name_kana);
  END IF;

  -- 代表者カナ も同様
  IF NEW.representative_name_kana IS NOT NULL THEN
    NEW.representative_name_kana := normalize_kana_zenkaku(NEW.representative_name_kana);
  END IF;

  RETURN NEW;
END $$;

CREATE TRIGGER trg_fruit_companies_legal_normalize
  BEFORE INSERT OR UPDATE OF phone, name_kana, representative_name_kana
  ON fruit_companies_legal
  FOR EACH ROW
  EXECUTE FUNCTION trg_fruit_companies_legal_normalize();
```

### 13.4 既存 Kintone 重複列の廃止

Kintone で「電話番号」と「電話番号（ハイフン除去）」の 2 列があった場合:

- Garden Fruit では `phone` 1 列のみ
- 取込時に正規化して保存
- 検索 / コピペは正規化済の値で実施（必要なら表示時にハイフン付与）

### 13.5 GENERATED vs trigger の選択基準

| 条件 | 選択 |
|---|---|
| IMMUTABLE 関数のみで変換可（regexp_replace 等）| GENERATED ALWAYS AS ... STORED |
| カスタム関数 / 多段変換 | trigger（BEFORE INSERT/UPDATE）|
| current_date / now() 含む | VIEW（GENERATED 不可）|
| 検索インデックス必要 | GENERATED + GIN/B-tree |

### 13.6 受入基準への追加

- [ ] corporate_number_normalized / invoice_number_normalized GENERATED 列動作
- [ ] name_search GIN インデックスで全文検索
- [ ] 電話番号 / カナ trigger 動作（INSERT/UPDATE 時 normalize）
- [ ] Kintone 重複列が 1 列に統合（取込時正規化）
- [ ] dev で 6 法人取込後、normalized 列が正しく生成

### 13.7 判断保留事項追加

| # | 論点 | a-auto スタンス |
|---|---|---|
| Gen-1 | normalize_kana_zenkaku() 実装方法 | PostgreSQL 拡張 / SQL 関数 / 取込時アプリ層変換 のいずれか、Phase B で確定 |
| Gen-2 | GENERATED 列のスキーマ変更 | Postgres は変換式変更不可 → DROP + ADD で再生成 |
| Gen-3 | trigger 連鎖の防止 | normalize trigger は INSERT/UPDATE 1 回のみ、再帰しない |
