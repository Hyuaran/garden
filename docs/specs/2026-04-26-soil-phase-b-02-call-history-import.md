...
B-02 段階では**全件ホット保持**で OK。アーカイブは運用 6-12 ヶ月後に判断。
...
Tree 側に Garden カレンダー連動の `feature_flags.tree_insert_enabled = false` を用意（既存 spec-cross-feature-flags 連動）。投入完了後に `true` 戻す。
...
| # | 論点 | a-auto スタンス |
|---|---|---|
| 判 1 | 投入実行時刻 | **土曜深夜 23:00 開始**、日曜朝完了想定 |
| 判 2 | INDEX 後付けの並列度 | 最大 2 並列、それ以上は I/O 飽和 |
| 判 3 | 24 ヶ月超アーカイブ即実施 | 不要（運用 6-12 ヶ月後に判断）|
