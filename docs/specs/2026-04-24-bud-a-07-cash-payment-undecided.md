# Bud A-07: 手渡し現金支給 仕様確定版

> **改訂 2026-04-25 (a-bud)**: 東海林さん 5 論点全採択完了、確定版に更新（旧タイトル「未決事項整理」）。
> ファイル名は履歴保護のため `*-undecided.md` のまま維持（git 参照整合性）、内部タイトルのみ確定版に変更。

- 対象: 給与を銀行振込ではなく**現金手渡し**で受取る従業員の扱い
- 見積: **0.25d**（業務ヒアリング + 合意形成、2026-04-25 完了）
- 担当セッション: a-bud
- 作成: 2026-04-24（a-auto / Phase A 先行 batch5 #A-07）
- 確定: 2026-04-25 a-main 経由で東海林さん全採択
- 元資料: Bud CLAUDE.md「⚠️ 未決事項（Phase 3 着手前に東海林と方針決定）」, `05_Garden-Bud/未決事項_20260419.md`
- 採択結果概要: `docs/bud-a07-hearing-items.md` §「東海林判断: 採択結果」
...
## 3. 論点と選択肢（採択結果反映済 2026-04-25）

> **🎯 全 5 論点 即決完了 (2026-04-25)**: 4 件は a-auto 推奨通り、1 件（論点 3）はメール配信追加で拡張採択。
> 採択結果サマリは `docs/bud-a07-hearing-items.md` §「東海林判断: 採択結果」を参照。
...
**✅ 採択結果 (2026-04-25)**: **A 案採択 + メール送信拡張**

東海林さんから A 案推奨にメール配信が追加されて拡張採択：

- **配信方式**: **方式 2 パスワード保護 PDF 添付**
...
実装ヒント（Phase B 着手時に再相談）:
- PDF 生成: `@react-pdf/renderer`（既存依存）
- パスワード保護: `bcryptjs` / `pdf-lib`（npm 追加要）
...
**✅ 採択結果 (2026-04-25)**: **A + B 併用採択（現金支給者限定、推奨通り）**

### 未確認事項（東海林さんに要ヒアリング → 暫定推測で合意済 2026-04-25）

| # | 未確認 | a-auto の推測 → 暫定合意 |
|---|---|---|
| U1 | 現在何名が手渡しか | 1〜3 名（小規模）と想定、確定要 → **暫定合意（業務情報追加待ち）** |
| U2 | どの法人の誰か | 法人単位で集中していない想定、ランダムの可能性 → **暫定合意** |
| U3 | 現金原資の引出しルール（担当・金額予測・タイミング）| 月末に経理担当が ATM で引出と推測、実運用確認要 → **暫定合意** |
| U4 | MF クラウド給与での計算対象か | 業務委託は対象外とされているので、手渡し **正社員** は MF 計算対象と推測 → **暫定合意** |
| U5 | 給与日（手渡し日）が振込日と同じか別か | 同日想定、ズレがある場合は別運用 → **暫定合意** |

**注**: 5 件すべて Phase B 着手前（2026-07 頃）までに東海林さんから業務情報を受領して確定する。緊急性なし。

---

## 4. データモデル（採択結果反映済 2026-04-25）

### 4.1 `root_employees` 追加カラム（論点 1 採択 = A 案）
```sql
...
### 4.2 `bud_transfers.transfer_type` enum 拡張（論点 2 採択 = B 案）
```sql
...
-- CSV 出力時のフィルタ:
-- WHERE transfer_type IS DISTINCT FROM '給与(手渡し)'
```

### 4.3 `bud_statements` カテゴリ定数（論点 4 採択 = A+C 併用、A-06 連携）
```sql
-- category で '現金原資引出' を予約カテゴリに
-- A-06 §4.1 の bud_statements.category の運用に追加
-- 月次集計画面（A-06 W7 既実装）で「給与(手渡し)合計」と「現金原資引出合計」を比較表示
```

### 4.4 受領確認テーブル（論点 5 採択 = A+B 併用）

**FK 型注意**: `bud_transfers` の実 PK は `transfer_id text` のため修正済（spec 初版の `id uuid` から訂正）。
...
-- 重複登録禁止（1 振込に対し 1 受領確認）
CREATE UNIQUE INDEX uq_bud_cash_payment_receipts_transfer
  ON bud_cash_payment_receipts (transfer_id);
```

### 4.5 給与明細メール配信（論点 3 拡張採択、Phase B B-03 で実装）

**新規テーブル提案**（Phase B 着手時に確定）:
```sql
CREATE TABLE bud_salary_pdf_deliveries (
  id                   uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  employee_id          text NOT NULL REFERENCES root_employees(employee_id),
  salary_month         date NOT NULL,                                  -- 月の 1 日（例: 2026-07-01）
  email_to             text NOT NULL,                                  -- 配信先（マイページ登録の email スナップショット）
  pdf_password_type    text NOT NULL CHECK (pdf_password_type IN ('birthday_mmdd', 'employee_no_last4')),
  delivery_status      text NOT NULL CHECK (delivery_status IN ('queued', 'sent', 'failed', 'bounced')),
  delivery_provider    text,                                           -- 'resend' / 'sendgrid' 等（Phase B 着手時に確定）
  sent_at              timestamptz,
  error_message        text,
  created_at           timestamptz NOT NULL DEFAULT now()
);

CREATE UNIQUE INDEX uq_bud_salary_pdf_deliveries_emp_month
  ON bud_salary_pdf_deliveries (employee_id, salary_month);
```

実装ヒント:
- PDF 生成: `@react-pdf/renderer`（既存依存）
- パスワード保護: `bcryptjs` / `pdf-lib`（Phase B で追加検討）
- メール送信: Resend / SendGrid 等（東海林さん相談事項）
- PW タイプは実装時に最終決定（生年月日 4 桁 / 社員番号下 4 桁）
