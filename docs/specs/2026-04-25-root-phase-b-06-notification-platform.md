...
- 見積: **2.75d**（Phase B-6.1〜B-6.3 合計）
- 担当セッション: a-root
- 作成: 2026-04-25（a-root / Phase B-6 spec）
- 最終更新: 2026-04-26（a-main 006 確定ログ反映）
...
本 spec は `decisions-root-phase-b-20260426-a-main-006.md` の **B-6 関連項目** を反映する。

### B-6 で確定した主要事項

| # | 項目 | 確定内容 | 反映先 |
|---|---|---|---|
| #2 | Email 統合: ハイブリッド構成（Microsoft + Resend）+ DKIM/SPF/DMARC 整備 | 業務メール = **Microsoft 365 / Outlook（既存契約）**経由、トランザクションメール（給与明細・通知等） = **Resend**（新規導入）。両者で **DKIM / SPF / DMARC** 整備して送信ドメイン認証 | §1 スコープ / §3 アーキテクチャ / §4 データモデル / §10 Phase 段階分け |
...
- **Microsoft 365 + Resend のハイブリッド Email 配信基盤**（§4.4 参照）
...
flowchart TB
    ...
    subgraph チャネル実装層
        ...
        F[sendEmail()]
        ER[emailRouter]
        MS[Microsoft 365<br/>SMTP / Graph API]
        RS[Resend API]
...
### 4.4 Email チャネルの実装方針（ハイブリッド構成）

#### 配信先別ルーティング

| 用途 | プロバイダ | DNS / 認証 | 備考 |
|---|---|---|---|
| 業務メール（社員間連絡、外部問い合わせ返信、案件相談）| **Microsoft 365 / Outlook** | DKIM / SPF / DMARC（既存契約のドメイン）| 既存運用継続 |
| トランザクションメール（給与明細、KoT 同期エラー、Chatwork 不達 fallback）| **Resend** | DKIM / SPF / DMARC（新規発行サブドメイン: `notify.hyuaran.com`）| Phase B-6.3 で新規導入 |

#### `root_notification_channels` の Email チャネル登録例

```sql
-- channel_type='email', config で provider を区別
-- channel_type='email', config={"provider": "microsoft", "from": "info@hyuaran.com"}
-- channel_type='email', config={"provider": "resend", "from": "noreply@notify.hyuaran.com"}
INSERT INTO root_notification_channels
  (channel_id, channel_type, display_name, config) VALUES
  ('email-admin-ms',
   'email',
   '管理者メール（業務）',
   '{"provider": "microsoft", "from": "info@hyuaran.com"}'),
  ('email-notify-resend',
   'email',
   'トランザクション通知メール',
   '{"provider": "resend", "from": "noreply@notify.hyuaran.com"}');
  -- Phase B-6.3 で追加
```
...
export async function sendEmail({
  ...
  if (provider === 'microsoft') {
    return sendViaMicrosoftGraph({ to, subject, body, from: channel.config.from });
  }
  if (provider === 'resend') {
    return sendViaResend({ to, subject, body, from: channel.config.from });
  }
  throw new Error(`unknown email provider: ${provider}`);
}
```

#### DNS 認証整備（Phase B-6.3 で実施）

- **Microsoft 365 既存ドメイン**: 既存 DKIM / SPF / DMARC を確認し、Resend と DMARC ポリシーが競合しないよう調整
- **Resend 新規サブドメイン（`notify.hyuaran.com`）**:
  - DKIM: Resend 提供の CNAME を DNS に追加
  - SPF: `v=spf1 include:_spf.resend.com ~all`
  - DMARC: `v=DMARC1; p=quarantine; rua=mailto:postmaster@hyuaran.com`
  - DMARC レポート受信先設定（不正送信の早期検知）

#### コスト試算（Phase B-6.3 着手前確認済）

| プロバイダ | 費用 | 根拠 |
|---|---|---|
| Microsoft 365 | 追加費用なし | 既存社員ライセンス内 |
| Resend | 月 100 通まで無料、以降 $20/月で 5 万通 | 給与明細配信 月 ~50 通 + KoT エラー通知 月 ~10 通 ≈ 60 通 → 当面無料枠内 |
...
### Phase B-6.3: Email 統合（ハイブリッド構成） — 0.75d

**目標**: Microsoft 365（業務メール）と Resend（トランザクションメール）のハイブリッド Email 配信基盤を整備する。

- [ ] Microsoft 365 SMTP / Graph API クライアント実装（業務メール用）
- [ ] Resend クライアント実装（トランザクションメール用）
- [ ] DNS / DKIM / SPF / DMARC 整備（Resend 新規サブドメイン `notify.hyuaran.com`）
- [ ] `sendEmail()` 統合ルーター実装（`provider` フィールドで振り分け）
- [ ] `email-admin-ms` / `email-notify-resend` チャネル追加（§4.4 参照）
- [ ] Bounce / Complaint Webhook エンドポイント（Resend 向け）
- [ ] 個人設定に Email ON/OFF 追加
- [ ] Bud B-03 給与明細フロー（DM ルーム未登録時の Email fallback）連携
...
| W11 | Microsoft 365 Graph API クライアント実装 | B-6.3 | 0.15d |
| W12 | Resend クライアント実装 + `sendEmail()` 統合ルーター | B-6.3 | 0.15d |
| W13 | DNS / DKIM / SPF / DMARC 整備（Resend サブドメイン）| B-6.3 | 0.1d |
| W14 | Bounce Webhook エンドポイント | B-6.3 | 0.15d |
| W15 | 個人設定 Email ON/OFF + Bud B-03 fallback 連携 | B-6.3 | 0.15d |
| W16 | 疎通テスト・dry-run 確認・段階リリース | 全体 | 0.25d |
| **合計** | | | **2.75d** |
...
| ~~判2~~ | ~~Email 基盤のプロバイダ選定~~ | **✅ 解消済み（2026-04-26 a-main 006 確定）**: 業務メール = Microsoft 365 / Outlook（既存契約）、トランザクションメール = Resend（新規導入）のハイブリッド構成に決定。§4.4 参照 |
...
| 判5 | 配信失敗時の admin 通知ループ防止 | chatwork-audit → 失敗時は `email-notify-resend` フォールバック。Email も未設定の場合は `root_notification_log.status='dead'` 記録のみ |
...
| ~~U3~~ | ~~Email 送信の必要性と既存契約~~ | **✅ 解消済み（2026-04-26 a-main 006 確定）**: 業務メール = Microsoft 365（既存契約）、トランザクション = Resend（新規導入）のハイブリッド構成に決定 |
