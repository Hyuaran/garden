...
- 背景: **青空系グラデーション**（ライト `#87CEEB → #E0F6FF`、ダーク `#1a3a52 → #0d1e2e`）
- 高さ: 64px
...
青空系はデフォルト。時間帯テーマ ON 時は下記に変化（04 参照）：

| 時間帯 | ヘッダー背景 |
|---|---|
| 朝焼け（06-09）| オレンジピンク `#FFB88C → #FFE5D9` |
| 昼かんかん（09-16）| 青空（既定） |
| 夕焼け（16-19）| 赤紫 `#FF6B6B → #FFA07A` |
| 夜（19-06）| 紺 `#0D1B2A → #1B2838` |
| ランダム | 日次シード + 上記 4 種からランダム |
...
:root {
  ...
  --header-sky-from: #87CEEB;
  --header-sky-to: #E0F6FF;
...
.dark {
  ...
  --header-sky-from: #1a3a52;
  --header-sky-to: #0d1e2e;
...
| UI-04 time-theme | `--header-sky-*` CSS var を時間帯で動的上書き |
