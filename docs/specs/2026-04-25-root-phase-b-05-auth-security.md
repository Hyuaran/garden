...
本 spec は `decisions-root-phase-b-20260426-a-main-006.md` の **B-5 関連項目** を反映する。

### B-5 で確定した主要事項

| # | 項目 | 確定内容 | 反映先 |
|---|---|---|---|
| #4 | パスワード再発行時のセッション処理: D 案採用 | 旧パスワードで取得した既存 JWT/refresh token を **revoke**（強制ログアウト）+ **不正検知ログ**（root_login_attempts に suspicious=true）+ **管理者通知**（Chatwork critical ルーム）の三本柱 | §3.1 / §3.4 / §5 / §9 |
...
#### パスワード変更時のセッション無効化（D 案、2026-04-26 確定）

パスワード変更操作（self-service / admin 強制 reset / forgot password）が成功した場合、以下の **D 案** 三本柱を実施する。

##### A. 旧 JWT / refresh token の revoke

- Supabase Auth の `auth.admin.signOut(user_id)` を **changed_at 時点まで遡って** revoke
- 現在のセッション（変更操作実施中）も含めて全 device で強制ログアウト
- revoke 対象: refresh token（実 JWT は短寿命のため失効待ち）

```typescript
// src/app/root/_actions/password-change.ts
...
export async function changePassword({ userId, newPassword }: ...) {
  ...
  // D 案 ① 全セッション revoke
  await admin.auth.admin.signOut(userId);  // 全 refresh token 失効

  // D 案 ② 不正検知ログ
  await admin.from('root_login_attempts').insert({
    user_id: userId,
    attempt_type: 'password_changed',
    suspicious: false,  // 自分での変更は false、admin 強制 reset は true
  ...
  await notify({
    ...
    payload: { userId, changedBy: currentUser.id, isForceReset: false }
  });
}
```

##### B. 不正検知ログ（root_login_attempts への記録）

- パスワード変更イベントを `root_login_attempts` に `attempt_type='password_changed'` で記録
- 不審判定（suspicious=true）の条件:
  - admin による強制 reset
  - 短期間に複数回のパスワード変更
  - パスワード変更後すぐに別 IP からログイン試行
- 監査ログ (B-2) にも `module='root', action='password_changed', severity='warn'` で記録

##### C. 管理者通知（Chatwork critical ルーム）

- 全パスワード変更イベントを `chatwork-audit-critical` ルームに通知
- フォーマット: 「YYYY-MM-DD HH:MM - {user_name}({employee_id}) のパスワードが変更されました。変更者: {changed_by}。次回ログイン時に強制ログアウト発動」
- 不審判定 true の場合は東海林さん DM にも転送

##### 適用範囲

- self-service パスワード変更
- admin による強制 reset
- forgot password フロー
- パスワード期限切れ強制変更（Phase B-5.X）

##### 適用対象外

- 初期パスワード（MMDD）から自分で変更する初回ログインフロー（既存セッションがないため）
...
#### D 案対応: `root_login_attempts` への追加カラム（2026-04-26 確定）

パスワード変更イベントを記録するため、以下のカラムを追加する（§3.1 D 案 B. に対応）。

```sql
ALTER TABLE root_login_attempts
  ADD COLUMN IF NOT EXISTS attempt_type text NOT NULL DEFAULT 'login'
    CHECK (attempt_type IN ('login', 'login_failed', 'password_changed', 'password_reset_requested', 'mfa_failed')),
...
| カラム | 型 | 用途 |
|---|---|---|
| `attempt_type` | text (ENUM 相当) | ログイン試行種別。既存の `success` / `failure_reason` との使い分けは: `attempt_type='login'` が従来のログイン試行、`'password_changed'` が D 案で追記するイベント |
| `suspicious` | boolean | 不審判定フラグ。admin 強制 reset や短期間複数回変更時に `true` を立てる |
| `changed_by` | uuid FK | パスワード変更の実施者（admin reset 時は admin の user_id、self-service 時は自分自身） |
...
// D 案処理の内部フロー（実装ガイド）
// 1. newPassword の強度チェック（ロール別要件）
// 2. currentPassword の照合（本人操作時）
// 3. root_password_history で過去 5 件再利用チェック
// 4. Supabase Auth updateUserById でパスワード更新
// 5. [D 案 ①] admin.auth.admin.signOut(targetUserId) で全セッション revoke
// 6. [D 案 ②] root_login_attempts に attempt_type='password_changed' で記録
//    - suspicious: isForceReset が true の場合は true
// 7. [D 案 ③] Chatwork critical ルームに通知
//    - isForceReset=true または suspicious=true の場合は東海林さん DM にも転送
// 8. root_password_history に新ハッシュを追記
// 9. root_audit_log に action='password_changed', severity='critical' で記録
// 10. root_employees.password_changed_at / force_password_reset を更新
```
...
### D 案: パスワード変更イベントの通知要件（2026-04-26 確定）

`password_changed` および `admin_password_reset` イベントは **critical 通知必須** とし、以下の通知経路を強制実施する。

| イベント | root_audit_log | Chatwork critical ルーム | 東海林さん DM |
|---|---|---|---|
| self-service パスワード変更 | ○ (severity='critical') | ○ | — |
| admin による強制 reset | ○ (severity='critical') | ○ | ○ |
| suspicious=true 判定時 | ○ (severity='critical') | ○ | ○ |

通知タイミング: `changePassword()` Server Action 内で同期実施（失敗時はログに記録して続行、通知失敗はパスワード変更処理をロールバックしない）。
