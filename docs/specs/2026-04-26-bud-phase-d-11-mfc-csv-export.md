...
- 改訂:
  - 3 次 follow-up (2026-04-26): 4 → 6 段階フロー（社労士確認段階追加、D-10 と同期）
  - **4 次 follow-up (2026-04-26)**: **6 → 7 段階**（⑤ visual_double_checked = 上田目視ダブルチェック）+ Cat 4 #26 / **#27 出力タイミング** 反映
- 前提:
  - **東海林判断 (2026-04-26)**: MFC CSV 仕様判明（72 列 / cp932 / 9 カテゴリ）
  - **東海林判断 (2026-04-26 4 次, Cat 4 #27)**: **MFC CSV 出力 = 振込ファイル生成と同時**（D-07 §4.1 ハイブリッド出力フローと連動）
...
- 関連 spec:
  ...
  - D-07（振込）— **MFC CSV 出力は振込ファイル生成と同時に実行**（Cat 4 #27、4 次 follow-up）
  - D-09（口座分離）— 振込先参照
...
### 1.4 出力タイミング（Cat 4 #27 反映、4 次 follow-up）

> **🎯 4 次 follow-up（Cat 4 #27、A 採択）**: **MFC CSV 出力は振込ファイル生成と同時に実行**する。

#### タイミング統一の理由

| 旧運用 | 新運用（4 次 follow-up）|
|---|---|
| MFC CSV 出力 → 後で振込ファイル生成 | **同時に両方を生成**（同一 Server Action 内）|
| 2 工程に分離 → 工程間遅延・整合性ズレリスク | 1 トランザクションで両方完結 → 整合性保証 |

#### 同時生成の実装（D-07 §4.1 連動）

```typescript
// D-07 §4.1 のハイブリッド出力フローに統合
async function exportPayrollBatch(input: { payrollPeriodId: string; companyId: string }) {
  // 経路 A: 全銀協 FB データ（D-07）
  const fbDataPath = await generateFbData(batchId);

  // 経路 B: 勘定項目別レポート CSV（D-07 §3.3）
  const accountingReport = await generateAccountingReport(batchId);

  // 経路 C: MFC CSV（本 spec、Cat 4 #27 で同時実行）
  const mfcCsv = await generateMfcCsv({ payPeriod, payDate });

  // 3 ファイルを 1 トランザクションで Storage 保存 + status='exported' 一括更新
  return { fbDataPath, accountingReport, mfcCsv };
}
```

#### Server Action の改訂（§5 と整合）

`generateMfcCsv` Server Action は単独でも呼び出し可能だが、D-07 ハイブリッド出力フローからの**同時呼び出しが標準**となる。
詳細は **D-07 §4.1 / §4.4**（FB データ生成と同時の MFC CSV 出力）を参照。
...
CREATE TABLE bud.mfc_csv_exports (
  ...
  -- ステータス（2026-04-26 [a-bud] 4 次 follow-up: 6 → 7 段階、D-10 と完全整合 + Cat 4 #26 反映）
  -- ⑤ visual_double_checked = 上田君による金額・氏名・口座の目視ダブルチェック（D-10 と同期）
  status text NOT NULL CHECK (status IN (
    ...
    'visual_double_checked',   -- ⑤ payroll_visual_checker（上田）目視ダブルチェック OK ⭐ NEW 4 次
  ...
  visual_double_check_requested_at timestamptz,       -- ⑤ 上田に目視ダブルチェック依頼時刻 ⭐ NEW 4 次
  visual_double_checked_at timestamptz,               -- ⑤ 上田が OK 戻した時刻 ⭐ NEW 4 次
  visual_double_checked_by uuid REFERENCES root.employees(id), -- ⑤ 通常 = 上田 ⭐ NEW 4 次
  visual_check_note text,                             -- ⑤ 上田の特記事項 / NG 内容 ⭐ NEW 4 次
  sharoshi_request_sent_at timestamptz,               -- ⑥ 社労士確認依頼送信時刻
  sharoshi_partner_id uuid REFERENCES root.partners(id),  -- 依頼先 root_partners
...
## 6. ステータス遷移（2026-04-26 [a-bud] 4 次 follow-up: 6 段階 → 7 段階に拡張、D-10 と整合）
...
confirmed_by_auditor（東海林目視 OK）
  ↓ payroll_auditor が "上田に目視ダブルチェック依頼" ボタン押下
  ↓     → Garden 内通知（上田画面に "未チェック" として登録）
  ↓ ④ payroll_visual_checker（上田）が金額・氏名・口座を 1 件ずつ目視
visual_double_checked（上田の目視ダブルチェック OK） ⭐ NEW 4 次 follow-up
...
特殊ケース:
- 上田目視 NG → `confirmed_by_auditor` 据え置き（status 変えず visual_check_note に NG 記録、東海林さん再判断）
- 社労士 NG → `visual_double_checked` に巻き戻し（再目視 → 必要なら CSV 再生成 → MFC 再取込）
- D-10 `bud.payroll_records` と本テーブルは status 同期（同一 batch_id ベースで片方を更新したら他方も更新）

> **後道さん不在の確認**: 4 次 follow-up（Cat 4 #26）で確定の通り、本ステータス遷移に**後道さんは登場しない**。確認系は東海林さん（payroll_auditor）+ 上田（payroll_visual_checker）+ 社労士（root_partners 外部）で完結する。

---

## 7. RLS（#18 反映、5 ロール、4 次 follow-up で 7 段階に拡張）

```sql
ALTER TABLE bud.mfc_csv_exports ENABLE ROW LEVEL SECURITY;

-- SELECT: payroll_* 全員（payroll_visual_checker 含む）
...
-- ④-依頼 confirmed_by_auditor (status 据え置き) → visual_double_check_requested_at セット ⭐ NEW 4 次
CREATE POLICY mfc_request_visual_check ON bud.mfc_csv_exports FOR UPDATE
  USING (status = 'confirmed_by_auditor' AND has_payroll_role(ARRAY['payroll_auditor']))
  WITH CHECK (
    status = 'confirmed_by_auditor'  -- status 変えず、visual_double_check_requested_at のみ更新
    AND has_payroll_role(ARRAY['payroll_auditor'])
  );

-- ④ confirmed_by_auditor → visual_double_checked: payroll_visual_checker（上田）⭐ NEW 4 次
-- 上田は SELECT + 当該遷移のみ、編集権なし
CREATE POLICY mfc_visual_check ON bud.mfc_csv_exports FOR UPDATE
  USING (
