# Bud Phase D #04: 給与明細配信（PDF 生成 + Tree マイページ主経路 + メール PW PDF 補助経路 + 上田目視ダブルチェック UI）

- 対象: Garden-Bud Phase D 給与明細・賞与明細の PDF 生成と配信 + 上田君用目視ダブルチェック UI
- 優先度: **🔴 高**（従業員 UX、法定要件、PII 取扱）
- 見積: **1.8d**（旧 1.5d + 4 次 follow-up 上田 UI 実装 +0.3d）
...
- 改訂:
  ...
  - **2026-04-26（a-bud 4 次 follow-up、Cat 4 #26 反映）**: **上田君「目視ダブルチェック」UI 要件（§2.7）正本** + 後道さん不在明記
- 前提:
  ...
  - **Cat 4 #26 採択結果（2026-04-26 a-main 確定）**: 給与確認フロー = 後道さん不在 + 上田君目視ダブルチェック + 東海林さん振込（§2.7、本 spec が UI 要件の正本リファレンス）
...
### 2.7 上田君「目視ダブルチェック」UI 要件（Cat 4 #26 反映、4 次 follow-up）⭐ 必須

> **🎯 4 次 follow-up（Cat 4 #26、修正採択）**
> 給与確認フローは **後道さん不在 + 上田君目視ダブルチェック + 東海林さん振込** で確定。
> 旧運用にあった「後道さんが目視確認」工程は**廃止**（Garden 上の確認フローに後道さんは不在）。
> 確定根拠: `decisions-pending-batch-20260426.md` Cat 4 #26 + `decisions-kintone-batch-20260426-a-main-006.md`
> 関連 memory: `project_payslip_distribution_design.md`（a-main 側で更新済）

#### 給与確認フロー確定版

```
東海林さんが Garden Bud で給与計算結果を確認（D-10 ④ confirmed_by_auditor）
   ↓
東海林さんが Garden 上で「上田に目視ダブルチェック依頼」ボタン押下
   ↓ Garden 内通知（KPIHeader 通知センター + 上田画面に "未チェック" 登録）
上田君が金額・氏名・口座を 1 件ずつ目視確認（時間かかってもよい）
   ↓ 全件チェック完了
上田君が Garden 内「確認 OK」ボタン押下
   ↓ status='visual_double_checked'（D-10/11 ⑤）
東海林さんが社労士確認依頼 → 社労士 OK → 確定処理
   ↓
東海林さんが実際の振込実行（finalized）
```

#### 上田君用 UI 要件（**本 spec が UI 要件の正本リファレンス**、D-10 §6.3 / D-07 §X から本セクションを参照）

| 項目 | 要件 |
|---|---|
| UI 複雑度 | **シンプル** — リスト + 行ごとのチェックボックス + 全件 OK 後に「確認 OK」ボタンのみ |
| 表示内容 | 金額・氏名・口座番号を **大きく見やすく**（フォント拡大、行間広く、スクリーン拡大対応）|
| 行レイアウト | 1 従業員 = 1 行。氏名（カナ + 漢字）/ 銀行コード + 支店 / 口座番号（普通/当座 + 番号）/ 金額（カンマ区切り、円表示）|
| 表示順 | 従業員番号昇順（上田君が業務上慣れている順序）|
| 時間制約 | **自動 timeout なし、急かす UI なし、丁寧に確認できる**（セッションタイムアウトの長時間設定 or 操作中の延長 keep-alive）|
| 操作権限 | 閲覧 + 各行の目視チェックマーク + 全件 OK 後に「確認 OK」ボタンのみ |
| 編集権限 | **なし**（金額・氏名・口座いずれも編集不可。誤りに気づいたら「差戻し（NG 戻し）」ボタンで `confirmed_by_auditor` に据え置きしながら note 記録 → 東海林さん再判断）|
| 配信・実行権限 | **なし**（編集・配信・振込は東海林さん専任）|
| 通知 | Garden 内 KPIHeader 通知センターに「上田: 給与目視チェック未消化 N 件」バッジ表示。Chatwork DM は使わない（Garden 内完結）|
| 上田君のスペック前提 | 複雑な操作不可、絶対間違えてはいけない目視チェック係、時間かかってもきっちり |

#### 上田君用画面の URL

`/bud/payroll/visual-check`（一覧）+ `/bud/payroll/visual-check/[batchId]`（個別 batch の目視 UI）

#### UI モック（個別 batch 画面）

```
┌─ 給与目視ダブルチェック（2026-05-25 支給分）──────────────┐
│                                                          │
│ 担当: 上田 基人                                            │
│ 全 N 件中 〇 件チェック済み                                │
│                                                          │
...
│ └──────────────────────────────────────────────────────┘ │
│                                                          │
│ [全件 OK 確認 → 「確認 OK」]  ← 全行チェック後に活性化     │
│ [差戻し（NG 戻し）]                                       │
└──────────────────────────────────────────────────────────┘
```

#### Server Action 連携（D-10 §6.6）

- 「確認 OK」ボタン → `markVisualDoubleChecked({ payrollRecordIds })` 呼出
- 「差戻し」ボタン → `noteVisualCheckIssue({ payrollRecordIds, reason })` 呼出（status 据え置き）
- 詳細仕様: D-10 §6.6 / §6.3 を参照

#### 後道さん不在の確認

旧運用にあった「後道さんが給与確認」工程は**廃止**:
- `bud_payroll_records` の status 遷移に「後道」関連の stage はない
- 上田 UI も「後道」へのアサイン箇所は持たない
- 後道さんへの会計レポート共有は別系統（D-07 §3.3 `bud_payroll_accounting_reports.shared_with_godo_at`）で継続
