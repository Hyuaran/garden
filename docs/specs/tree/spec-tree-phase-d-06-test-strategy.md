...
- 改訂: **2026-04-26（a-main 006）— 判断保留 7 件すべて確定 + テスト用社員番号規則（test01-test09）正式化**
- 前提:
  - D-01 〜 D-05 の全 spec（2026-04-26 改訂版）
  - 新規 spec-tree-softphone-design / spec-tree-toast-notification（テスト対象に追加）
...
| # | 項目 | 確定内容 |
|---|---|---|
| 0-1 | 負荷試験ツール | **k6（Supabase 公式推奨、Phase D-1 で初導入）** |
| 0-2 | カバレッジ未達 PR の扱い | **禁止（🔴 厳守、Tree = 最厳格モジュール）。Leaf 🟡 より厳しい。CI で block merge** |
| 0-3 | Playwright 物理キーボード検証 | **`headless: false` 実ブラウザ + CI で xvfb（Phase D-1 で初導入）** |
| 0-4 | FM vs Garden 統計照合の自動化 | **自動化実装（Python + FM ODBC + Supabase）** |
| 0-5 | α版での録音聴取テスト | **α版除外、β段階で録音仕様確定後追加**（イノベラ API 仕様待ちと整合） |
| 0-6 | 本番 rollback の実行権限 | **東海林さん + admin 2 人承認**（二重承認で誤発動防止） |
| 0-7 | テスト用ダミー社員番号 | **`test01` 〜 `test09`**（実在 0001-0009 = 東海林さん 0008 含むため、`test` プレフィックスで識別） |

### テストデータ識別子の命名規則（Garden 全体統一）

判 0-7 を Garden 全体に拡張：
- **社員番号**: `test01`〜`test99`（実在の数字社員番号と区別）
- **顧客 ID**: `TEST_*`（例: `TEST_KANDEN_001`）
- **その他データ**: `dummy_*` プレフィックス（例: `dummy_call_record_001`）
- 本番 DB ではこれらを **DELETE 対象として識別可能**にする（`WHERE id LIKE 'test%' OR id LIKE 'TEST_%' OR id LIKE 'dummy_%'`）

### 既存実装との整合性
- 既存 Tree Phase A / B / B-β のテスト資産（Vitest / RTL）は本 spec の 🔴 最厳格基準に再評価し、不足分を追加
- 既存 `/tree/alerts` 等の有効率（eff）ロジックは本 spec の Vitest で 100% カバレッジ目標
- 新規 `spec-tree-softphone-design.md` / `spec-tree-toast-notification.md` のテストも本 spec の Playwright E2E に組込
...
- **判1（確定）: 負荷試験ツール** — 確定: k6（Supabase 公式推奨、Phase D-1 で初導入）（推定通り）
- **判2（確定）: カバレッジ未達 PR** — 確定: 禁止（🔴 厳守、CI で block merge）（推定通り）
- **判3（確定）: Playwright 物理キー検証** — 確定: `headless: false` 実ブラウザ + CI で xvfb（推定通り）
- **判4（確定）: FM vs Garden 統計照合** — 確定: 自動化実装（Python + FM ODBC + Supabase）（推定通り）
- **判5（確定）: α版録音聴取テスト** — 確定: α版除外、β段階で録音仕様（イノベラ API）確定後追加（推定通り）
- **判6（確定）: 本番 rollback 権限** — 確定: 東海林さん + admin 2 人承認（推定通り）
- **判7（確定）: テスト用ダミー社員番号** — **確定: `test01`〜`test09`** — 当初推定（`0001-0009`）から **`test` プレフィックス方式へ変更**（実在 0008 = 東海林さんと衝突回避）。Garden 全体のテストデータ命名規則（§0）に統合
