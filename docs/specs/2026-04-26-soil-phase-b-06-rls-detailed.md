...
## 5. 案件単位可視範囲（通常テーブル `soil_lists_assignments`）

### 5.1 構造（§6.4 採用方針に基づく通常テーブル、2026-04-27 a-review R-3 反映）

> **【重要】**: 本テーブルは **MV ではなく通常テーブル**。理由は §6 で詳述（MV はトリガで部分更新できない Postgres 制約）。
> Batch 16 §5.2 / 旧版では MV と記載していたが、Phase B 実装着手版では **§6.4 採用フロー（通常テーブル + トリガ即時更新 + 1h fallback cron）に統一**。
> B-04 §6.3 / §7 の MV 表記とも整合（同改訂で全箇所書換）。

```sql
-- §6.4 採用方針: 通常テーブル + トリガ即時更新（B-04 §6.3 も同方式）
...
-- 効率化用部分インデックス（§17.4.2 ポリシーの WHERE / EXISTS 句と完全同形の式で作成）
-- ⚠ Postgres expression INDEX は同一の式 + 同一の演算子で参照されないと使われない。
--   §17.5 / §17.6 の EXPLAIN ANALYZE で "Index Scan using idx_root_employees_module_owner_kanden" を必須確認。
...
-- フォールバック / 将来モジュール拡張時の汎用 INDEX（部分 INDEX が効かないケース対策）
CREATE INDEX idx_root_employees_module_owner_flags_gin
  ON root_employees USING gin (module_owner_flags jsonb_path_ops);
```

##### EXPLAIN ANALYZE 検証（必須、2026-04-27 a-review R-5 反映）

§17.4.2 ポリシー実行時に部分 INDEX が確実に利用されることを検証する。

```sql
-- 槙さん（leaf_kanden owner）相当の auth.uid() を mock した SELECT に対して
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM leaf_kanden_cases
...
```
->  Index Scan using idx_root_employees_module_owner_kanden on root_employees
      Index Cond: ((module_owner_flags ->> 'leaf_kanden'::text) = 'true'::text)
```

`Seq Scan on root_employees` が出る場合は INDEX 未利用 → 部分 INDEX の式 / WHERE 句が §17.4.2 の式と完全一致しているか再点検（演算子クラス / cast / 型推論差異の可能性）。GIN INDEX のみ利用される場合も性能差を §17.6 で計測。
...
- [ ] §17.4.1 EXPLAIN ANALYZE で `idx_root_employees_module_owner_kanden` 部分 INDEX が利用されることを確認（2026-04-27 a-review R-5 追加）
