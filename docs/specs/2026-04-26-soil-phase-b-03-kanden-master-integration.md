...
> **2026-04-26 更新（a-main-007 batch）**: 33 件 判断保留消化のうち Soil 担当 5 件（Cat 5 #29-#33）を東海林さん採択にて確定。
> 確定対象 = 判 1 / 判 4 / 判 5 / 判 8 / 判 9（全件 a-auto スタンスどおり）。
> 出典: `C:\garden\_shared\decisions\decisions-pending-batch-20260426.md` §カテゴリ 5

| # | 論点 | a-auto スタンス | 状態 |
|---|---|---|---|
| 判 1 | R1 supply_point_22 が NULL の Kintone レコード | R2 以降にフォールバック、admin 警告 | **確定**（2026-04-26、Cat 5 #29、a-auto 推奨どおり）|
| 判 2 | Kintone 削除検知の頻度 | 月次（毎月 1 日）、削除確定は連続 2 月確認後 | 保留 |
| 判 3 | リスト外の `source_channel` 既定値 | 'walk_in'、UI で変更可 | 保留 |
| 判 4 | makinaoshi 判定基準 | admin 手動のみ、自動判定しない | **確定**（2026-04-26、Cat 5 #30、a-auto 推奨どおり、システム自動判定なし）|
| 判 5 | orphan 警告の即時 DM 抑止 | 同種違反は 5 分集約、初回のみ即時 | **確定**（2026-04-26、Cat 5 #31、a-auto 推奨どおり）|
| 判 6 | テストレコード判定の追加条件 | TEST_ プレフィックス + 'テスト太郎' 名 + 000-0000-0000 電話 | 保留 |
| 判 7 | merge 取消期限 | **24h** | 保留 |
| 判 8 | リスト外取込開始時刻 | **03:00 JST**（B-02 と統一）| **確定**（2026-04-26、Cat 5 #32、a-auto 推奨どおり）|
| 判 9 | 旧月次バッチ（飛び込み補完）の cleanup | 既存実装あれば削除、未実装なら無視 | **確定**（2026-04-26、Cat 5 #33、a-auto 推奨どおり）|
