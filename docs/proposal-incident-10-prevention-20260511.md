...
```
| **a-review** | コード/PR レビュー + UI mock 視覚評価兼任（5/9 確定）。**判断粒度は「構造比較」止まりが標準、「実描画確認」を求める場合は main 側で明示依頼必須。review self-prep「構造比較で完結」表記時は main 側で実描画確認の追加要求必須（違反 10 再発防止、5/11 強化）**|
```
