...
2. claude.ai が dispatch 形式（~~~ ラップ）+ HTML 全文をコードブロックで応答
   例:
   ~~~
   🟢 forest-html-N
   【claude.ai chat から a-main-015 への dispatch（tab-N 起草完了 + HTML 全文転送）】
   発信日時: HH:MM
   件名: tab-N-XXX.html 起草完了、a-main-015 配置代行依頼
   
   # HTML 全文（a-main-015 が _chat_workspace/garden-forest/html_drafts/tab-N-XXX.html に書き込み）
   ```html
   <!DOCTYPE html>
   ...（全文）
   ```
...
4. a-main-015 が HTML 全文を Drive に書き込み
5. a-main-015 が Read で評価 → OK なら次タブ進行 / NG なら修正方向 dispatch
```
