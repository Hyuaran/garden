-- Bud Phase D 13 migration merged SQL v5 (RLS exists 縮退版、has_role_at_least + auth_employee_number 採用)
-- 起草: a-main-023 / 2026-05-11 19:46
-- bud-002 commit f8a52b1 (RLS exists 縮退 43 件完走)
-- 検証: user_id / garden_role / select id / re.deleted_at 全て 0 件
...
create policy pr_approve on public.bud_payroll_records
  ...
  using (
    status = 'calculated'
    and public.bud_has_payroll_role(array['payroll_approver'])
    and calculated_by <> public.auth_employee_number()
  )
  with check (
    status = 'approved'
    and public.bud_has_payroll_role(array['payroll_approver'])
