...
### 🔴 最重要：D-1 + D-2 セットリリース戦略（2026-04-26 a-main 確定 #16、新案 D 採択）

**従来案（廃止）**：D-1（基本架電）リリース → 1 ヶ月後に D-2（自動発信 + ソフトフォン）追加リリース
**新案 D（採択）**：**D-1 と D-2 をセットで完成させ、テスト稼働で品質確保 → 全体展開で完全切替**

#### 採択の根拠
- D-1 だけ先行リリース → 1 ヶ月後に D-2 追加、というやり方は、現場が「便利機能の後付け」を待つ期間が無駄
- D-1 リリース直後の混乱期に 2 機能が混在すると「どっちで困っているか」が不明確
- → セットで完成させ、テスト期間を厳格に取って品質確保してから全体展開が最適解
- FileMaker 切替は失敗許されないので、初回投入時に「便利機能込みの完全版」を出すのが信頼性向上
- memory `project_tree_d2_release_strategy.md` 参照

#### 開発・展開タイムライン（本プラン §9-§10 を本戦略で更新）

| 段階 | 内容 |
|---|---|
| 開発期間 | D-1 完成後すぐ D-2 着手、両機能を一体で完成させる（本プラン §3-§7 に D-2 ソフトフォン spec 実装を統合） |
| α 版（東海林 1 人）| §16 7 種テスト完走 + UI / 操作性確認、**D-1 + D-2 両方含む** |
...
**Tree D-1 に含む（既存プラン §3 D-02 operator-ui）**:
- リスト選択 → 番号表示 → 手動発信
- コール履歴記録（`tree_call_records`）
- 結果入力（成果・不在・断り 等の result_code）
- マイページ・KPI 表示

**Tree D-2 に含む（追加スコープ、本戦略で D-1 と一体化）**:
- 電話番号クリックで即発信（D-1 の手動発信を不要化）
- ソフトフォン Garden 内構築（X-Lite 簡素化版、`spec-tree-softphone-design.md`）
- 発信中の通話制御（ミュート・転送・終話・モニタリング）
- 発信前の確認 dialog（誤発信防止）

#### 工数見積への影響

本戦略採択により、ソフトフォン実装（spec-tree-softphone-design.md 見積 2.0d）を **本プラン §3 D-02 operator-ui に統合**。プラン全体の見積を以下に修正：

| 旧見積 | 新見積（D-1+D-2 セット）| 備考 |
|--:|--:|---|
| 6.5d（D-1 のみ） | **8.5d（D-1+D-2 セット）** | +2.0d ソフトフォン実装 |

実装着手後、§3 D-02 operator-ui の Task 18-30 の中でソフトフォン Provider + 電話番号フックポイントを完成させる（イノベラ API 仕様書受領前提）。仕様書未受領なら本プラン全体を待機。

#### 関連判断との整合

- **#17 モニタリング切替**：D-2 リリース時に同時 50 名超なら WebSocket 化、超えなければ 30 秒 polling 維持（D-03 spec §0 に追記）
- **#18 録音聴取**：D-1+D-2 では Garden 内実装せず、イノベラへの誘導のみ（API 採用次第で再判断、注記付き）
- **#19 他商材展開**：D-1+D-2 セット完了後に **D-2.5 で光、D-3 でクレカ**（D-04 spec §0 に追記）
