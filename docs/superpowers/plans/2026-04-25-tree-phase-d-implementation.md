...
## 6. 判断保留事項（東海林さん要確認、合計 39 件 / 確定 3 件 + 残り 36 件）

> spec 起草時点で a-auto が「仮スタンス」を提示済の項目。実装着手前または α テスト中に確定が望ましいもの。
>
> **v3.1 改訂（2026-05-11）**: §6.1（確定 3 件、main- No. 223 で東海林さん GO）+ §6.2（残り 36 件、順次解消）に分割。
> 集計訂正: v3 ヘッダー「合計 38 件」は集計ミス、実数 39 件（D-01:3 / D-02:7 / D-03:7 / D-04:7 / D-05:8 / D-06:7）。確定 3 件を除いて残り 36 件。

### 6.1 確定判断（3 件、2026-05-11 GO）

| # | 内容 | 確定内容 | 確定経緯 |
|---|---|---|---|
| 02-判2 | オフラインキュー上限 | **500 件で確定**、超過 = 業務停止扱い（Toast 警告）。現場運用後に調整可。 | main- No. 223（a-main-020 から a-tree-002）|
| 04-判4 | 同意確認の全商材必須化 | **全商材必須で確定**（景表法・特商法準拠、安全側）。 | 同上 |
| 06-判6 | L3 承認者（Tree テーブル DROP + FM 再 sync）| **東海林さん + admin 2 名の dual 承認で確定**。 | 同上 |

→ 実装着手時は §6.1 の 3 件 = 確定済として進行可。

### 6.2 残り判断保留（36 件、順次解消）
...
- §6.1 の 3 件 = **既確定**、実装時はそのまま採用
- §6.2 の 36 件 = 各 § 着手前に main- No. 後続候補で 4 列テーブル提示 → 東海林さん決裁 → 確定項目を §6.1 へ昇格
- α テスト中に発見された判断事項は β 移行前にまとめて確定
- 否認・修正項目は plan v3.1 を v3.2 に更新（spec 修正は別 PR で a-auto に依頼）

### 6.4 Phase D 着手前提条件（v3.1 追加）

実装着手前に以下の整備が必須:

| 前提 | 状態（2026-05-11 時点） | 責任 |
|---|---|---|
| Phase D plan v3 確定 | ✅（4/25 起草、70 task / 6.5d）| a-tree（完了）|
| 急務 3 件 判断保留 | ✅（§6.1 確定）| 東海林さん（完了、main- No. 223）|
| 残り 36 件 判断保留 | 🟡 実装着手と並行で順次解消（§6.2）| a-tree → main → 東海林さん |
| **Batch 7 cross-cutting 関数 Supabase apply** | 🟡 未 apply、横断調査中 | **a-main-020 主導**（a-root-002 / a-soil-002 / a-bud-002 経由、main- No. 230）|
| PR #31（Phase D 6 spec）develop merge | ✅ merge 済（commit a4c932f）| a-main（完了）|

→ **Batch 7 関数 apply が Phase D §0 Pre-flight 着手の最終ブロッカー**。`auth_employee_number()` / `has_role_at_least()` / `is_same_department()` が garden-dev に存在しない場合、D-01 schema 適用時に CHECK 制約参照エラーが発生する（D-01 schema は本関数群を RLS / WITH CHECK で使用）。

apply 完了後、Phase D §0 Pre-flight Task 0 から順次着手可能。
