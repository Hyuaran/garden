...
**Goal (v2 2026-04-25 改訂):** 関電業務委託案件の画像（電灯/動力/ガス/諸元/受領書）を Garden-Leaf 上で閲覧・アップロード・圧縮・サムネ生成・**論理削除 + UNDO + 復元 + 物理削除 + 画像 DL（スマホ時パスワード認証）**できるようにする。client 主導 + **8 ロール × 事業スコープ RLS** + 3 バケット分離の構成。

**Architecture:** 3 層構造（UI / ロジック / インフラ）。UI は Backoffice（PC 事務用）と Input（モバイル営業用）の 2 画面。ロジックは `src/app/leaf/_lib/` に集約、`attachments.ts`（9 関数）/ `image-compression.ts` + Worker / `kanden-storage-paths.ts` / `role-context.tsx`（v2 新規、ロール全体共有）。インフラは Supabase Storage × 3 bucket + Postgres テーブル 3 本（`leaf_kanden_attachments` 拡張 + `leaf_businesses` + `leaf_user_businesses` 新規）。spec-cross-rls-audit §2 パターン A に従い `src/lib/supabase/client.ts` を新設して Leaf を先頭バッターとする。**Root A-3-g（`is_user_active` / `garden_role_of` / `outsource` ロール / `contract_end_on` 列）に依存**。
...
**仕様書**: `docs/superpowers/specs/2026-04-23-leaf-a1c-attachment-design.md`（**v2 2026-04-25 改訂**、a-bloom 技術レビュー v1 分 §2🟢/§4🟢/§7🟡/§8🟢、**v2 は東海林さん再レビュー待ち**）

**見積 (v2)**: **6.0d**（Phase 0 = 0.3d / Phase D 共通基盤 = **2.5d** / Phase A Backoffice = **2.0d** / Phase B Input = **1.5d** / Phase F 仕上げ = 0.2d、並行一部あり）※ v1 比 +2.0d

**前提依存**: Root A-3-g migration（`scripts/root-schema-patch-a3g.sql`、`feature/root-phase-a3-g-employees-outsource-extension`）が develop にマージ・Supabase Dashboard 実行済みであること。未マージなら本 plan の Phase D-1 で停止。
...
**インフラ / Migration:**
- `scripts/leaf-schema-patch-a1c.sql` — ALTER TABLE + leaf_businesses/leaf_user_businesses 新規テーブル + leaf_user_in_business 関数 + 3 bucket + **8 ロール × 事業スコープ RLS ポリシー**
...
- `attachments.ts` — **9 関数**: upload / signedURL / softDelete / undoSoftDelete / **restoreAttachment (v2)** / **hardDeleteAttachment (v2)** / **verifyUserPasswordAndDownload (v2)** / **isMobileDevice (v2)** / **getCurrentGardenRole (v2)**
...
- `AttachmentCard.tsx` — 共通サムネカード + **削除済バッジ表示 (v2)**
- `AttachmentGrid.tsx` — サムネ一覧（カテゴリ別タブ）+ **削除済分類 (v2)**
- `AttachmentLightbox.tsx` — 1500px 拡大ビューワ + **DownloadButton 組込 (v2)**
- `AttachmentUploader.tsx` — PC 向け drag&drop + file input
- `AttachmentDeleteButton.tsx` — **manager+ のみ表示 (v2)**、ホバー × + 2 段確認 + UNDO
- `DownloadButton.tsx` — **v2 新規**: PC 即 DL / スマホ パスワードモーダル + 3 回ロック
- `AttachmentAdminActions.tsx` — **v2 新規**: 削除済カードに admin+ 限定「復元」「完全削除」
...
- `attachments.test.ts` (v2 で +5 関数分のケース追加)
- `role-context.test.tsx` — **v2 新規**

**テスト `src/app/leaf/backoffice/_components/__tests__/`:**
- `AttachmentGrid.test.tsx` (v2 で削除済バッジ + ロール別 UI のテスト追加)
- `AttachmentUploader.test.tsx`
- `AttachmentLightbox.test.tsx`
- `AttachmentDeleteButton.test.tsx` (v2 でロール別表示テスト追加)
- `DownloadButton.test.tsx` — **v2 新規**
- `AttachmentAdminActions.test.tsx` — **v2 新規**
...
### Task D.1: Migration SQL v2（手動実行 + 記録、事業スコープ + 8 ロール RLS）
...
/** 論理削除（deleted_at = now() + deleted_by = auth.uid()、v2 改訂）
 *
 * Client 側で manager+ ロールガード必須。本関数は RLS の事業所属チェックで
 * 呼出者を絞り込むが、論理削除の列制御は RLS では困難のため UI 層で防ぐ。
 */
...
### Task D.13: `attachments.ts` v2 追加関数群（restoreAttachment / hardDeleteAttachment / verifyUserPasswordAndDownload / isMobileDevice / getCurrentGardenRole）
...
### Task A.3: `AttachmentDeleteButton.tsx` + RTL test（ロール判定 + 2 段確認 + UNDO）

**[v2 改訂追記]** 本コンポーネントは **manager 以上のみ × を表示する**。実装時:
- `useGardenRole()` で現在ロールを取得（D.12 で実装済）
- `!DELETABLE_ROLES.includes(role)` なら `return null`（UI 出さない）
- RTL テストに「toss ロールでは × が表示されない」シナリオを追加
- 既存の 3 テスト（confirm 表示 / onDelete 呼出 / cancel）はそのまま

修正後の RTL テスト例（追加ケース）:

```typescript
import { RoleProvider } from '@/app/leaf/_lib/role-context';
// useGardenRole をモック: vi.mock('@/app/leaf/_lib/role-context', () => ({...}))

it('is hidden for toss role', () => {
  vi.mocked(useGardenRole).mockReturnValue('toss');
  render(<AttachmentDeleteButton attachmentId="a1" onDelete={vi.fn()} />);
...
import { DELETABLE_ROLES } from '@/app/leaf/_lib/types';

export function AttachmentDeleteButton({ attachmentId, onDelete }: Props) {
  const role = useGardenRole();
  if (!role || !DELETABLE_ROLES.includes(role)) return null;
  // 既存の confirm UI
}
```
...
**（以下、v1 plan の内容をそのまま実装して、上記の改訂を追記する）**
...
### Task A.3b: `DownloadButton.tsx` + RTL test（v2 新規、PC 即 DL / スマホ PW 認証）
...
|---|---|---|---|
| Phase 0 | 3 | 3 | 0 |
| Phase D | 12 | **14** (+ D.12 RoleContext / D.13 v2 関数群) | **+2** |
| Phase A | 6 | **8** (+ A.3b DownloadButton / A.3c AttachmentAdminActions) | **+2** |
| Phase B | 3 | 3 | 0 |
| Phase F | 5 | 5 | 0 |
| **合計** | **29** | **33** | **+4** |
...
|---|---|---|---|---|
| Phase 0 | 0.3d | 0.3d | 0 | - |
| Phase D | 1.5d | **2.5d** | **+1.0d** | 事業マスタ migration / RoleContext / v2 関数群 5 個 / RLS 複雑化 / supabase-mock 拡張 |
| Phase A | 1.5d | **2.0d** | **+0.5d** | DownloadButton / AttachmentAdminActions / Grid の削除済分類 / DeleteButton ロール判定 |
| Phase B | 1.0d | **1.5d** | **+0.5d** | スマホ DL PW 実機試験 / RoleContext provider 組込 |
| Phase F | 0.2d | 0.2d | 0 | 20 シナリオ化は Phase F 範囲内で吸収 |
| **合計** | **4.0d** | **6.0d** | **+2.0d** | - |
