...
### ⌨️ キーボードショートカット（FileMaker風UXの限定適用）

Forest は基本「見るだけ」のダッシュボードのため、FileMaker 風の検索・結果セット操作（`Ctrl+F`・演算子検索等）は **不要**。  
ただし進行期データ編集モーダルでは、月次更新時に 6 法人分を連続編集することを想定し、以下のショートカットを実装する:

| キー | 動作 |
|---|---|
| `Ctrl+↑` / `Ctrl+↓` | 前/次の法人の編集モーダルへ移動（`sort_order` 順） |
| `Esc` | モーダルを閉じる |
| `Ctrl+S` | 保存（モーダルは開いたまま） |
| `Ctrl+Enter` | 保存して次の法人へ移動 |

**関連メモリ**: `.claude/memory/project_garden_filemaker_ux.md`
