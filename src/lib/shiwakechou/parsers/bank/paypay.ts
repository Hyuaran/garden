 ...
 * PayPay 銀行 CSV パーサー
 *
 * 入力フォーマット (実 CSV ヘッダー検証済):
 *   "操作日(年)","操作日(月)","操作日(日)","操作時刻(時)","操作時刻(分)","操作時刻(秒)",
 *   "取引順番号","摘要","お支払金額","お預り金額","残高","メモ"
 *
 * 仕様:
...
// 実 CSV: "操作日(年)","操作日(月)","操作日(日)" で始まる
// 防御的に "取引日" / "操作日" の両方を許容
const HEADER_PATTERN = /(?:操作日|取引日).*年.*(?:操作日|取引日).*月.*(?:操作日|取引日).*日/;
