...
import { assertOwnConfirmationName, isMailState, type MailWriteOperation } from "./write-ops";
...
export async function patchOneMessage(request: Request, id: string, op: MailWriteOperation) {
  try {
    ...
    if (op === "confirm" && body.name !== undefined) {
      ...
      try { assertOwnConfirmationName(String(body.name), data.name); }
      catch { throw new RillMailHttpError(403, "他の利用者の確認印は変更できません"); }
    }
