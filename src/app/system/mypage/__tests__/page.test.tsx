...
describe("system mypage", () => {
  ...
  it("submits the expanded emergency contact payload with the employee name read-only",async()=>{const fetchMock=vi.fn().mockResolvedValue(new Response(JSON.stringify({ok:true,rows:[]}),{status:200}));vi.stubGlobal("fetch",fetchMock);renderMyPage({...baseProps,birthdayRegistered:false,initialProfile:{...profile,birthday:null}});fireEvent.click(screen.getByRole("button",{name:/緊急連絡先変更/}));expect(screen.getByLabelText("提出者本人の氏名")).toHaveValue("社員A");expect(screen.getByLabelText("提出者本人の氏名")).toHaveAttribute("readonly");for(const [label,value] of [["現住所","東京都"],["個人の電話番号（携帯等）","090"],["緊急連絡先の氏名","家族A"],["本人との続柄","母"],["緊急連絡先の住所（同一の場合は「同上」）","同上"],["緊急連絡先の電話番号","080"]])fireEvent.change(screen.getByLabelText(label),{target:{value}});fetchMock.mockResolvedValueOnce(new Response(JSON.stringify({ok:true,id:"1"}),{status:201}));fireEvent.click(screen.getByRole("button",{name:"送信する"}));await waitFor(()=>expect(fetchMock).toHaveBeenCalledWith("/api/system/mypage/submissions",expect.objectContaining({method:"POST"})));const call=fetchMock.mock.calls.find(([url,init])=>url==="/api/system/mypage/submissions"&&init?.method==="POST");expect(JSON.parse(String(call?.[1]?.body))).toEqual({type:"emergency_contact",payload:{kind:"new",selfAddress:"東京都",selfPhone:"090",ecName:"家族A",ecRelationship:"母",ecAddress:"同上",ecPhone:"080"}})});
