...
export default function ZenkakuTab({ runCheck = runZenkakuCheck, postalDataStatus = null, now = new Date() }: { runCheck?: (salesId: string) => Promise<GardenCheckResult>; postalDataStatus?: PostalDatasetStatus | null; now?: Date }) {
  ...
  async function pollSubmission(id:string){for(let i=0;i<60;i++){await new Promise(r=>setTimeout(r,1000));const response=await fetch(`/api/system/zenkaku-submit/${id}`,{cache:"no-store"});const state=await response.json();if(state.status==="done"){setSubmitMessage(`前確依頼を出しました（案件ID ${state.case_id}）`);return;}if(state.status==="needs_confirmation"){const count=state.candidates?.[0]?.count??result?.duplicateCount??0;if(!window.confirm(`既に${count}件あります。追加で出しますか？`)){setSubmitMessage("前確依頼を取りやめました。");return;}await fetch("/api/system/zenkaku-submit",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({submissionId:id,confirmDuplicate:true})});continue;}if(state.status==="needs_partner"){const labels=(state.candidates??[]).map((c:{code:string;label:string},n:number)=>`${n+1}: ${c.label}（${c.code}）`).join("\n");const answer=window.prompt(`どちらのチームか選んでください。\n${labels}`);const chosen=state.candidates?.[Number(answer)-1];if(!chosen){setSubmitMessage("チームを選択してから、もう一度お試しください。");return;}await fetch("/api/system/zenkaku-submit",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({submissionId:id,partnerCode:chosen.code})});continue;}if(state.status==="failed"){const messages:Record<string,string>={team_not_registered:"チームが名簿に未登録です。管理者へ連絡してください。",duplicate_pending:"同じ商材の前確待ち案件が既にあります。",dropdown_mismatch:"商材の選択肢がKintoneにありません。管理者へ連絡してください。"};throw new Error(messages[state.error_code]||"前確依頼を登録できませんでした。時間をおいて、もう一度お試しください。");}}throw new Error("前確依頼の登録に時間がかかっています。時間をおいて状況をご確認ください。");}
  ...
  return <section className={styles.zenkakuContent} aria-labelledby="zenkaku-title">
    ...
    {canProceed ? <section className={`${styles.card} ${styles.successResult}`} aria-labelledby="success-title"><h2 id="success-title">確認できました</h2><p>前確依頼に必要な項目が入力されています。</p><button type="button" onClick={()=>void submit()} disabled={submitting}>{submitting?"前確依頼を送信中…":"前確依頼を出す"}</button>{submitMessage?<p role="status">{submitMessage}</p>:null}</section> : null}
  </section>;
}
