...
describe("ZenkakuTab", () => {
  ...
  it("summarizes three duplicates and shows one detail per line",async()=>{const duplicates=[{caseId:"L260001",productName:"光A",registeredDate:"2026-01-01"},{caseId:"L260002",productName:"光B",registeredDate:"2026-02-01"},{caseId:"L260003",productName:"光C",registeredDate:"2026-03-01"}];render(<ZenkakuTab runCheck={async()=>evaluateGardenCheck(createValidSalesMasterRecord(),duplicates)}/>);fireEvent.change(screen.getByLabelText("営業ID"),{target:{value:"L1"}});fireEvent.click(screen.getByRole("button",{name:"連携チェック"}));expect(await screen.findByText("この営業IDは既に3件登録されています。別商材の追加契約であれば、そのまま進めてください。")).toBeInTheDocument();for(const duplicate of duplicates)expect(screen.getByText(`${duplicate.caseId} ／ ${duplicate.productName} ／ ${duplicate.registeredDate}`)).toBeInTheDocument();expect(screen.queryByText(/この営業IDは既に登録されています（案件ID/)).not.toBeInTheDocument();});
  it("shows no warning when there are no duplicates",async()=>{render(<ZenkakuTab runCheck={async()=>evaluateGardenCheck(createValidSalesMasterRecord(),[])}/>);fireEvent.change(screen.getByLabelText("営業ID"),{target:{value:"L1"}});fireEvent.click(screen.getByRole("button",{name:"連携チェック"}));await screen.findByRole("heading",{name:"確認できました"});expect(screen.queryByRole("heading",{name:"警告"})).not.toBeInTheDocument();});
