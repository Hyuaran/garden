...
export default function OnboardingReview({ values, onEdit }: { values: OnboardingInput; onEdit?: (step: number) => void }) {
  return <div className={styles.review}>{STEPS.slice(0, 7).map((title, index) => <section key={title} aria-label={title}>
    ...
    {index === 6 && <p>{values.nda_agreed ? "内容を確認しました" : <span className={styles.empty}>未入力（同意の確認なし）</span>}</p>}
  </section>)}</div>;
}
