...
describe("オリエンテーション表示", () => {
  it("Shellのレール・サイドバー・上部バーを出さず戻るリンクを表示する", () => {
    render(<OrientationLayout><CompanyDocument members={members} presentation /></OrientationLayout>);
  ...
  it("通常表示と全6章・表示メンバー・写真が同じ", async () => {
    ...
    expect(expected.chapters).toHaveLength(6); expect(expected.members).toHaveLength(12); expect(expected.photos).toHaveLength(7);
    normal.unmount();
    const presentation = render(<OrientationLayout>{await OrientationPage()}</OrientationLayout>);
  ...
  it("通常表示の入口は説明文と線画SVGを持つ", () => {
    ...
    expect(screen.getByText("メニューを隠して、資料だけを大きく映します。")).toBeInTheDocument();
