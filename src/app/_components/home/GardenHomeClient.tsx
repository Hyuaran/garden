...
/**
 * GardenHomeClient — v2.8a Step 5 動的版を Task 2 で client component に分離
 *
 * 経緯:
 *   page.tsx は元々 "use client" だったが、Task 2 (plan §Step 2-2) で
 *   server component 化 (role 取得 + isHomeForbidden redirect) するため、
 *   既存 v2.8a Step 5 の client ロジックを行単位で本ファイルに移管した。
 *
 * 機能後退なし:
 ...
 *
 * Task 2 追加:
 *   - props: role / visibleModules
 *   - OrbGrid に visibleModules forward
 *   - Sidebar に visibleModules forward
 *
...
// ============================================================================
type Props = {
  /** server side で取得した garden_role（null = 未取得 / 不明）*/
  role?: string | null;
  /** server side で算出した可視 module 一覧。未指定なら全 12 module を描画（後方互換）*/
...
// ============================================================================
export default function GardenHomeClient({
  role: _role,
  visibleModules,
}: Props = {}) {
  // === Theme (light/dark) ===
  ...
  return (
    <>
      ...
      {/* Sidebar (左 210px、固定) — Task 2 で visibleModules filter */}
