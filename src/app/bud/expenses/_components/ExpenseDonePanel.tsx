...
import { buildEmployeeMap, fetchExpenseEmployeeLookup, resolveExpenseApplicantName, type ExpenseEmployeeLookupRow } from "../_lib/expense-employees";
...
export function ExpenseDonePanel() {
  ...
  const load = useCallback(async () => {
    setLoaded(false);
    let query = supabase.from("bud_expense_requests").select(SELECT).eq("status", "journalized").is("deleted_at", null).order("booking_date", { ascending: false, nullsFirst: false }).order("id", { ascending: true });
    ...
    const [requestRes, summaryRes, corpRes, categoryRes] = await Promise.all([
      ...
      supabase.rpc("bud_expense_done_summary", { p_booking_corp_id: corpId === "all" ? null : corpId, p_start: start, p_end: end }),
    ...
    const aggregate = ((summaryRes.data as DoneSummaryRpcRow[] | null) ?? [])[0];
    const count = Number(aggregate?.total_count ?? 0);
    const taxIncluded = Number(aggregate?.total_amount ?? 0);
    setRows(nextRows);
    setSummary({ count, taxIncluded, taxExcluded: calculateTaxExcludedAmount(taxIncluded) });
    ...
    setMessage(requestRes.error ? `読み込みに失敗しました: ${requestRes.error.message}` : summaryRes.error ? `集計に失敗しました: ${summaryRes.error.message}` : null);
    setLoaded(true);
...
const pagination: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "center", gap: 12, flexWrap: "wrap", marginTop: 14 };
const pagePosition: React.CSSProperties = { minWidth: 150, textAlign: "center", color: "var(--text-main)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontVariantNumeric: "tabular-nums" };
...
const ellipsis: React.CSSProperties = { ...td, maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis" }; const applicantCell: React.CSSProperties = { ...td, maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis" }; const numberCell: React.CSSProperties = { ...td, textAlign: "right", fontVariantNumeric: "tabular-nums" };
