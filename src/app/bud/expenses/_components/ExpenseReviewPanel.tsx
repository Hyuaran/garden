 ...
 * - embedded 時は、タブ内の既存3カード(.exp-sub-card)の数字を実データで更新
 *   （承認待ち=残件＋合計、承認/差戻し=本日0時以降の処理分）
 */
...
type TodayStats = { approvedCount: number; approvedAmount: number; rejectedCount: number; rejectedAmount: number };
...
export function ExpenseReviewPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const [today, setToday] = useState<TodayStats>({ approvedCount: 0, approvedAmount: 0, rejectedCount: 0, rejectedAmount: 0 });
  ...
  const loadToday = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .gte("keiri_checked_at", iso);
    const rows = (data as { amount: number | null; status: string }[] | null) ?? [];
    const approved = rows.filter((r) => r.status !== "keiri_returned" && r.status !== "submitted");
    const rejected = rows.filter((r) => r.status === "keiri_returned");
    setToday({
      approvedCount: approved.length,
      approvedAmount: approved.reduce((s, r) => s + (r.amount ?? 0), 0),
      rejectedCount: rejected.length,
      rejectedAmount: rejected.reduce((s, r) => s + (r.amount ?? 0), 0),
    });
  }, [supabase]);
  ...
  // embedded: タブ内の既存3カードの数字を実データで更新（表記は 件/円 のまま）
  useEffect(() => {
    ...
    set(cards[1], today.approvedCount, `合計 ${yen(today.approvedAmount)}`);
    set(cards[2], today.rejectedCount, yen(today.rejectedAmount));
  }, [embedded, loaded, list, today]);
      ...
      {!embedded && (
        <>
          ...
          <section style={cards3}>
            ...
            <Card label="承認（本日）" value={today.approvedCount} color="#5e7d44" />
            <Card label="差戻し（本日）" value={today.rejectedCount} color="#b35850" />
          </section>
        </>
      )}
...
const twoCol: React.CSSProperties = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "start", marginBottom: 20 };
