  ...
  return (
    <section className="mx-auto mt-6 max-w-6xl rounded-lg border border-amber-200 bg-white/80 p-5 shadow-sm">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="font-shippori text-lg font-semibold text-amber-950">
            支払区分別 一覧
          </h2>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {tabs.map((category) => (
          ...
          >
            ...
            <span className="ml-2 rounded-full bg-white/70 px-2 py-0.5 text-xs text-amber-900">
              {counts[category]}
            </span>
          </button>
        ))}
      ...
      {loading ? (
        <div className="rounded border border-amber-100 bg-amber-50/60 p-4 text-sm text-amber-800">
          読み込み中…
        </div>
            ...
            <table className="w-full min-w-[880px] border-collapse text-sm">
              <thead className="bg-amber-100/70 text-left text-xs text-amber-950">
                <tr>
                  ...
                  {activeTab === "payeasy" && (
                    <th className="px-4 py-3 font-medium">ペイジー番号</th>
                  )}
