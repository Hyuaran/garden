...
import { boardFilterValues, matchesBoardColumnFilters, type BoardColumnFilters, type BoardColumnKey } from "../_lib/board-filters";
...
export default function Board() {
  ...
  const [columnFilters,setColumnFilters]=useState<BoardColumnFilters>({}); const [sort,setSort]=useState<ColumnKey>("introducedAt"); const [direction,setDirection]=useState<"asc"|"desc">("desc");
  ...
  const columnValues=useMemo(()=>Object.fromEntries(columns.map(column=>[column.key,Array.from(new Set(rows.flatMap(row=>boardFilterValues(row,column.key)))).filter(value=>value!=="—").sort()])),[rows]);
  const visible=useMemo(()=>rows.filter(row=>{
    ...
    return (!search||haystack.includes(search.toLowerCase()))&&(!status||row.status===status)&&(!rank||row.rank===rank)&&(!product||row.products.includes(product))&&matchesBoardColumnFilters(row,columnFilters);
  ...
  return <section className={styles.board}>
    ...
    <div className={styles.sheet}><table><thead><tr>{columns.map((column,index)=><th key={column.key} className={index===0?styles.first:""} onClick={()=>changeSort(column.key)}><div className={styles.columnTitle}>{column.label}<span>{sort===column.key?(direction==="asc"?"↑":"↓"):"↕"}</span></div><ColumnFilter label={column.label} values={columnValues[column.key]||[]} selected={columnFilters[column.key]||[]} onChange={values=>setColumnFilters(current=>({...current,[column.key]:values}))}/></th>)}</tr></thead><tbody>{!loading&&!error&&visible.map(row=><tr key={row.id}>{columns.map((column,index)=><td key={column.key} className={index===0?styles.first:""}>{column.key==="status"?<span className={`${styles.badge} ${styles[row.status]}`}>{row.status}</span>:display(row[column.key])}{column.key==="latestActivity"&&row.latestCall&&<small className={styles.call}>{row.latestCall}</small>}</td>)}</tr>)}{!loading&&!error&&!visible.length&&<tr><td colSpan={columns.length} className={styles.empty}>該当する案件はありません</td></tr>}</tbody></table></div>
  </section>;
}
