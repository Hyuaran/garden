...
target as (
  select phone from input_phone
  union
  select "電話番号" from ledger where coalesce("電話番号", '') <> ''
  union
  select "携帯番号" from ledger where coalesce("携帯番号", '') <> ''
),
history as (
  ...
  join target t on t.phone = p."電話番号"
  union all
  ...
  join target t on t.phone = a."電話番号"
  union all
  ...
  join target t on t.phone = regexp_replace(coalesce(h.phone_number, ''), '\\D', '', 'g')
  union all
  ...
  join target t on t.phone = c."電話番号"
  where c."出所" = 'FM取込'
  union all
  ...
  join target t on t.phone = o."電話番号"
  union all
  ...
  join target t on t.phone in (
    regexp_replace(coalesce(s."電話番号", ''), '\\D', '', 'g'),
    regexp_replace(coalesce(s."携帯番号", ''), '\\D', '', 'g')
  )
  union all
  ...
  join target t on t.phone = b."電話番号"
)
