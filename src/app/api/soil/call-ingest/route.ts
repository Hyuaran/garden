...
import { parseCallIngestBody, CallIngestValidationError } from "@/app/soil/_lib/call-ingest";
...
export async function POST(request: Request) {
  ...
  const { data: log, error: logError } = await supabase.from("soil_call_sync_log").insert({
  ...
  if (logError || !log) {
    console.error("[soil/call-ingest] sync log start failed", logError?.message ?? "missing log id");
    return NextResponse.json({ ok: false, error: "同期ログを開始できません" }, { status: 500 });
  }

  const finishLog = async (values: Record<string, unknown>) => {
    const completedAt = new Date();
    const { error } = await supabase.from("soil_call_sync_log").update({
      ...values,
      completed_at: completedAt.toISOString(),
      duration_ms: Math.max(0, completedAt.getTime() - startedAt.getTime()),
    }).eq("id", log.id);
    if (error) console.error("[soil/call-ingest] sync log finish failed", error.message);
  };
  ...
  try {
    ...
    const { data: existing, error: existingError } = await supabase
      .from("soil_call_history").select("external_call_id").in("external_call_id", ids);
    ...
    const { error: upsertError } = await supabase.from("soil_call_history")
    ...
    console.error("[soil/call-ingest] batch failed", message, { run_id: parsed.metadata.runId, batch_index: parsed.metadata.batchIndex });
