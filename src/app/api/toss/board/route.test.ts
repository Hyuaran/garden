...
vi.mock("@/app/p/toss/_lib/kintone.server", () => ({ getRecords: mocks.getRecords }));
...
describe("GET /api/toss/board", () => {
  ...
  it("returns only rows whose trimmed partner code is in the Kanden roster", async () => {
    ...
    mocks.getRecords
      .mockResolvedValueOnce({ records: [tossRecord("1", "1234567"), tossRecord("2", "7654321"), tossRecord("3", "9999999")], totalCount: "3" })
      .mockResolvedValueOnce({ records: [rosterRecord(" 1234567 "), rosterRecord(7654321)] });
    ...
    expect(mocks.getRecords).toHaveBeenNthCalledWith(2, "38", "roster-token", 'ドロップダウン_2 in ("関電トス") limit 500');
  });
  ...
  it("combines the roster filter with the partner's mine filter", async () => {
    ...
    mocks.getRecords
      .mockResolvedValueOnce({ records: [tossRecord("1", "1234567"), tossRecord("2", "7654321")], totalCount: "2" })
      .mockResolvedValueOnce({ records: [rosterRecord("1234567"), rosterRecord("7654321")] });
  ...
  it("falls back to all existing rows and logs a warning when the roster lookup fails", async () => {
    ...
    mocks.requirePartnerOrStaff.mockResolvedValue({ kind: "staff" });
    mocks.getRecords
      .mockResolvedValueOnce({ records: [tossRecord("1", "1234567"), tossRecord("2", "9999999")], totalCount: "2" })
