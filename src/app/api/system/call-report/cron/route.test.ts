...
const mocks = vi.hoisted(() => ({ getSupabaseAdmin: vi.fn(), sendCallReportMessage: vi.fn() }));
vi.mock("@/lib/supabase/admin", () => ({ getSupabaseAdmin: mocks.getSupabaseAdmin }));
vi.mock("@/app/system/_lib/chatwork", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/app/system/_lib/chatwork")>()),
  sendCallReportMessage: mocks.sendCallReportMessage,
}));
...
describe("GET /api/system/call-report/cron", () => {
  beforeEach(() => {
    ...
    mocks.sendCallReportMessage.mockReset().mockResolvedValue({ ok: true });
