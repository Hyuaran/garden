...
const mocks=vi.hoisted(()=>({update:vi.fn(),maybeSingle:vi.fn()}));
vi.mock("@/lib/supabase/admin",()=>({getSupabaseAdmin:()=>({from:()=>({update:mocks.update})})}));
...
describe("POST /api/system/zenkaku-agent/result",()=>{
  beforeEach(()=>{process.env.ZENKAKU_AGENT_SECRET="secret";mocks.update.mockReset();mocks.maybeSingle.mockReset();mocks.update.mockImplementation((values)=>({eq:()=>({eq:()=>({select:()=>({maybeSingle:mocks.maybeSingle}),values})})}));mocks.maybeSingle.mockResolvedValue({data:{id},error:null});});
