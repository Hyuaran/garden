...
type BankRow = { bank_code: string; bank_name: string; valid_to?: string | null };
const responseBanks = (data: BankRow[] | null) => ({ banks: (data ?? []).map(row => ({ bankCode: row.bank_code, bankName: row.bank_name, expired: Boolean(row.valid_to), validTo: row.valid_to ?? null })) });
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (code) {
      const { data, error } = await supabase.from("system_bank_master").select("bank_code,bank_name,valid_to").eq("bank_code", code).order("bank_code", { ascending: true }).limit(1);
    ...
    for (const term of bankSearchTerms(name)) {
      const { data, error } = await supabase.from("system_bank_master").select("bank_code,bank_name,valid_to").is("valid_to", null).or(`bank_name.ilike.*${term}*,bank_kana.ilike.*${term}*`).order("bank_code", { ascending: true }).limit(20);
      if (error) throw error;
