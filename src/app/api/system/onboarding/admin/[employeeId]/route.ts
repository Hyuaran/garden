...
import { applyAdminOnboarding, onboardingAdminContext, saveAdminOnboarding } from "@/app/system/onboarding/_lib/onboarding-admin.server";
...
export async function POST(request: Request, { params }: { params: Promise<{ employeeId: string }> }) {
  ...
  try {
    ...
    if (!body || !["save", "apply"].includes(body.action)) throw new OnboardingError("保存方法を確認してください。", 400);
    if (body.action === "apply") await applyAdminOnboarding(context, employeeId, body.values);
    ...
    const message = error instanceof OnboardingError ? error.message : "保存できませんでした。もう一度お試しください。";
