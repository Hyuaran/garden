...
export async function POST(request:Request){
 ...
 const {data:employee}=await admin.from("root_employees").select("name").eq("user_id",auth.user.id).maybeSingle();
 const name=String(employee?.name||auth.user.user_metadata?.full_name||auth.user.user_metadata?.name||"担当者");
